...
namespace BoundTreeGenerator
{
    internal sealed class BoundNodeClassWriter_CS : BoundNodeClassWriter
    {
        ...
        private Action Braced(Action Content)=>() => InBlock(LBrace, Content ?? NotUsed(),true, RBrace);
        ...
        protected override void ImportsNamespaces()
        {
            ...
            ImportNamespace("Microsoft.CodeAnalysis.CSharp.Symbols");
            ImportNamespace("Microsoft.CodeAnalysis.CSharp.Syntax");
        }
        ...
        protected override void WriteKinds() =>  InBlock(() => WriteLine("internal enum BoundKind: byte"), Braced(ApplyToTreeNodes((node) => WriteLine($"{FixKeyword(StripBound(node.Name))},"))), false, null);
        ...
        private void Fa(string modifiers, string methodName, string[] parameters, string returns, Action basecall = null, string statement = null)
        {
            ...
            if (statement != null) Write($" => {statement}");
            EOL();
        }
              ...
              body: () =>
               {
                    ...
                    foreach (var field in Fields(node))
                        WriteLine($"this.{(IsPropertyOverrides(field) ? "" : "")}{field.Name} = {HandleField(node, field)};");
                });
        }
        ...
        protected override void WriteField(Field field)
            => WriteLine($"public {(IsPropertyOverrides(field) ?"override":"")} {(IsNew(field) ? "new " : "")}{field.Type} {field.Name} {{ get; }}");
                            ...
                            InBlock(
                                ...
                                Braced(
                                    ApplyToTreeNodes((node) =>
                                    {
                                        ...
                                        WriteLine($"case BoundKind.{FixKeyword(strip)}: return Visit{strip}(node as {node.Name}, arg);");
                                    })),
                                false,
                                footer: () => WriteLine("return default(R);"));
                             }); // end method
                    }); // end class
        ...
        private void ThisVisit(Field field) => WriteLine($"this.Visit{(IsNodeList(field.Type) ? "List" : "")}(node.{field.Name});");

        protected override void WriteWalker()
        {
            WriteClass("internal abstract partial","BoundTreeWalker","BoundTreeVisitor",
                ApplyToTreeNodes(node =>
                {
        ...
        {
            WriteClass("internal sealed","BoundTreeDumperNodeProducer","BoundTreeVisitor<object, TreeDumperNode>",
                ()=>{
        ...
        {
            Blank();
            WriteClass("internal abstract partial", "BoundTreeRewriter", "BoundTreeVisitor",
                    ApplyToTreeNodes(node =>
                    {
                        F("public override", $"Visit{StripBound(node.Name)}", Parameters(Parameter("node", node.Name)), "BoundNode",
                        body: () =>
                        {
                            ...
                            {
                                hadField = true;
                                WriteLine($"var {field.Name} = this.VisitType(node.{field.Name});");
                            }
                            if (hadField)
                            {
        ...
        {
            base.WriteClass(modifiers, classname, inherits, Braced(body), NotUsed(),false);
        }
