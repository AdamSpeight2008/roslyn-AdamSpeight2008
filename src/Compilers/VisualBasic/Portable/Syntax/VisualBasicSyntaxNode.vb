...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Partial Public MustInherit Class VisualBasicSyntaxNode
      ...
      Get
        If _syntaxTree Is Nothing Then
          ...
          While current IsNot Nothing
            ...
            If tree IsNot Nothing Then Exit While
          ...
          While stack.Count > 0
            ...
            If alternativeTree IsNot Nothing Then tree = alternativeTree
          End While
          stack.Free()
        End If
        Return _syntaxTree
    ...
    ''' <summary> Returns the <see cref="SyntaxKind"/> of the node. </summary>
    Public Function Kind() As SyntaxKind
      Return CType(Green.RawKind, SyntaxKind)
    End Function

    ''' <summary> The language name this node is syntax of. </summary>
...
#Region "Serialization"
    ...
    Public Shared Function DeserializeFrom(stream As IO.Stream, Optional cancellationToken As CancellationToken = Nothing) As SyntaxNode
      ...
      If Not stream.CanRead Then Throw New InvalidOperationException(CodeAnalysisResources.TheStreamCannotBeReadFrom)
      Using reader = ObjectReader.TryGetReader(stream, cancellationToken:=cancellationToken)
        If reader Is Nothing Then Throw New ArgumentException(CodeAnalysisResources.Stream_contains_invalid_data, NameOf(stream))
...
#End Region
    ...
    Friend Shared Function DoGetSyntaxErrors(tree As SyntaxTree, nodeOrToken As SyntaxNodeOrToken) As ReadOnlyCollection(Of Diagnostic)
      If Not nodeOrToken.ContainsDiagnostics Then Return EmptyErrorCollection
    ...
    Private Shared Sub PushNodesWithErrors(stack As Stack(Of SyntaxNodeOrToken), nodes As ChildSyntaxList)
      ...
      For Each n In nodes
        ...
        If n.ContainsDiagnostics Then stack.Push(n)
       Next
    End Sub
          ...
          Else
            ...
            For Each tr In child.AsToken.LeadingTrivia
              If tr.IsDirective Then
                ...
                If ((predicate Is Nothing) OrElse predicate(d)) Then Return d
              End If
            Next
            Continue For
          End If
...
#Region "Core Overloads"
    ...
    Protected Overrides Function ReplaceCore(Of TNode As SyntaxNode)(
      ...
      Return SyntaxReplacer.Replace(Me, nodes, computeReplacementNode, tokens, computeReplacementToken, trivia, computeReplacementTrivia).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function RemoveNodesCore(nodes As IEnumerable(Of SyntaxNode), options As SyntaxRemoveOptions) As SyntaxNode
      Return SyntaxNodeRemover.RemoveNodes(Me, nodes, options).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function ReplaceNodeInListCore(originalNode As SyntaxNode, replacementNodes As IEnumerable(Of SyntaxNode)) As SyntaxNode
      Return SyntaxReplacer.ReplaceNodeInList(Me, originalNode, replacementNodes).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function InsertNodesInListCore(nodeInList As SyntaxNode, nodesToInsert As IEnumerable(Of SyntaxNode), insertBefore As Boolean) As SyntaxNode
      Return SyntaxReplacer.InsertNodeInList(Me, nodeInList, nodesToInsert, insertBefore).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function ReplaceTokenInListCore(originalToken As SyntaxToken, newTokens As IEnumerable(Of SyntaxToken)) As SyntaxNode
      Return SyntaxReplacer.ReplaceTokenInList(Me, originalToken, newTokens).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function InsertTokensInListCore(originalToken As SyntaxToken, newTokens As IEnumerable(Of SyntaxToken), insertBefore As Boolean) As SyntaxNode
      Return SyntaxReplacer.InsertTokenInList(Me, originalToken, newTokens, insertBefore).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function ReplaceTriviaInListCore(originalTrivia As SyntaxTrivia, newTrivia As IEnumerable(Of SyntaxTrivia)) As SyntaxNode
      Return SyntaxReplacer.ReplaceTriviaInList(Me, originalTrivia, newTrivia).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function InsertTriviaInListCore(originalTrivia As SyntaxTrivia, newTrivia As IEnumerable(Of SyntaxTrivia), insertBefore As Boolean) As SyntaxNode
      Return SyntaxReplacer.InsertTriviaInList(Me, originalTrivia, newTrivia, insertBefore).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
    ...
    Protected Overrides Function NormalizeWhitespaceCore(indentation As String, eol As String, elasticTrivia As Boolean) As SyntaxNode
      Return SyntaxNormalizer.Normalize(Me, indentation, eol, elasticTrivia, useDefaultCasing:=False).AsRootOfNewTreeWithOptionsFrom(SyntaxTree)
    End Function
#End Region
    ...
    Public Shadows Function GetLocation() As Location
      ...
      If SyntaxTree IsNot Nothing Then
        Dim tree = SyntaxTree
        If tree.IsEmbeddedSyntaxTree Then Return New EmbeddedTreeLocation(tree.GetEmbeddedKind, Span)
        If tree.IsMyTemplate Then Return New MyTemplateLocation(tree, Span)
      End If
