...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax
  Public Class SyntaxNodeRemover
    ...
    Private Class SyntaxRemover     : Inherits VisualBasicSyntaxRewriter
      ...
      Friend ReadOnly Property ResidualTrivia As SyntaxTriviaList
        Get
          If _residualTrivia Is Nothing Then Return Nothing
      ...
      Private Sub AddEndOfLine()
        If _residualTrivia.Count = 0 OrElse Not IsEndOfLine(_residualTrivia(_residualTrivia.Count - 1)) Then _residualTrivia.Add(SyntaxFactory.CarriageReturnLineFeed)
      End Sub
      ...
      Public Overrides Function VisitList(Of TNode As SyntaxNode)(list As SeparatedSyntaxList(Of TNode)) As SeparatedSyntaxList(Of TNode)
        ...
        For i As Integer = 0 To n - 1
          ...
          If alternate IsNot Nothing AndAlso Not visited.IsKind(SyntaxKind.None) Then alternate.Add(visited)
        Next

        If alternate IsNot Nothing Then Return alternate.ToList().AsSeparatedList(Of TNode)()
      ...
      Private Sub AddTrivia(node As SyntaxNode)
        ...
        If (_options And (SyntaxRemoveOptions.KeepDirectives Or SyntaxRemoveOptions.KeepUnbalancedDirectives)) <> 0 Then AddDirectives(node, GetRemovedSpan(node.Span, node.FullSpan))
        ...
        If (_options And SyntaxRemoveOptions.AddElasticMarker) <> 0 Then AddResidualTrivia(SyntaxFactory.TriviaList(SyntaxFactory.ElasticMarker))
      End Sub
      ...
      Private Function GetRemovedSpan(span As TextSpan, fullSpan As TextSpan) As TextSpan
        ...
        If (_options And SyntaxRemoveOptions.KeepLeadingTrivia ) <> 0 Then removedSpan = TextSpan.FromBounds(span.Start, removedSpan.End)
        If (_options And SyntaxRemoveOptions.KeepTrailingTrivia) <> 0 Then removedSpan = TextSpan.FromBounds(removedSpan.Start, span.End)
        Return removedSpan
      End Function
      ...
      Private Sub AddDirectives(node As SyntaxNode, span As TextSpan)
        If node.ContainsDirectives Then
          ...
          Dim directivesInSpan = node.DescendantTrivia(span, Function(n) n.ContainsDirectives, descendIntoTrivia:=True).
          ...
          For Each directive In directivesInSpan
            ...
            ElseIf HasRelatedDirectives(directive) Then
              ...
              End If
            End If
            If _directivesToKeep.Contains(directive) Then AddResidualTrivia(SyntaxFactory.TriviaList(directive.ParentTrivia), requiresNewLine:=True)
          Next
        End If
      End Sub
      ...
      Private Shared Function HasRelatedDirectives(directive As DirectiveTriviaSyntax) As Boolean
        Select Case directive.Kind
          Case SyntaxKind.IfDirectiveTrivia,
               ...
               SyntaxKind.EndRegionDirectiveTrivia  : Return True
        End Select
        Return False
      End Function
    End Class
