...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Friend Module SyntaxNodeExtensions
    ...
    ''' <summary> Find enclosing WithStatement if it exists. </summary>
    ''' <param name="node"/>
    <Extension>
    ...
    Public Sub GetAncestors(Of T As VisualBasicSyntaxNode, C As VisualBasicSyntaxNode)(node As VisualBasicSyntaxNode, result As ArrayBuilder(Of T))
      ...
      Do While current IsNot Nothing AndAlso Not (TypeOf current Is C)
        If TypeOf current Is T Then result.Add(DirectCast(current, T))
    ...
    ''' <summary> Simplified version of ExtractAnonymousTypeMemberName implemented on inner tokens. </summary>
    <Extension>
...
TryAgain:
    ...
    Private Function GetCorrespondingConditionalAccessReceiver(node As ExpressionSyntax) As ExpressionSyntax
      ...
      If access IsNot Nothing Then Return access.Expression
      Return Nothing
    End Function
      ...
      While parent IsNot Nothing
        Select Case parent.Kind
          Case SyntaxKind.DictionaryAccessExpression,
            ...
            If DirectCast(parent, MemberAccessExpressionSyntax).Expression IsNot access Then Return Nothing
          Case SyntaxKind.XmlElementAccessExpression,
               SyntaxKind.XmlDescendantAccessExpression,
               SyntaxKind.XmlAttributeAccessExpression
            If DirectCast(parent, XmlMemberAccessExpressionSyntax).Base IsNot access Then Return Nothing
          Case SyntaxKind.InvocationExpression
            If DirectCast(parent, InvocationExpressionSyntax).Expression IsNot access Then Return Nothing
          Case SyntaxKind.ConditionalAccessExpression
            Dim conditional = DirectCast(parent, ConditionalAccessExpressionSyntax)
            If conditional.WhenNotNull Is access Then Return conditional
            If conditional.Expression IsNot access Then Return Nothing
          Case Else
            Return Nothing
        End Select
        access = parent
    ...
    ''' <summary> Returns true if all arguments are of the specified kind and they are also missing. </summary>
    <Extension>
    ...
    ''' <summary> Returns true if all arguments are missing. </summary>
    ...
    ''' <summary> Given a syntax node of query clause returns its leading keyword. </summary>
    <Extension>
    Public Function QueryClauseKeywordOrRangeVariableIdentifier(syntax As SyntaxNode) As SyntaxToken
      Select Case syntax.Kind
        Case SyntaxKind.CollectionRangeVariable : Return DirectCast(syntax, CollectionRangeVariableSyntax).Identifier.Identifier
        Case SyntaxKind.ExpressionRangeVariable : Return DirectCast(syntax, ExpressionRangeVariableSyntax).NameEquals.Identifier.Identifier
        Case SyntaxKind.FromClause              : Return DirectCast(syntax, FromClauseSyntax).FromKeyword
        Case SyntaxKind.FromClause              : Return DirectCast(syntax, FromClauseSyntax).FromKeyword
        Case SyntaxKind.LetClause               : Return DirectCast(syntax, LetClauseSyntax).LetKeyword
        Case SyntaxKind.AggregateClause         : Return DirectCast(syntax, AggregateClauseSyntax).AggregateKeyword
        Case SyntaxKind.DistinctClause          : Return DirectCast(syntax, DistinctClauseSyntax).DistinctKeyword
        Case SyntaxKind.WhereClause             : Return DirectCast(syntax, WhereClauseSyntax).WhereKeyword
        Case SyntaxKind.SkipWhileClause,
             SyntaxKind.TakeWhileClause         : Return DirectCast(syntax, PartitionWhileClauseSyntax).SkipOrTakeKeyword
        Case SyntaxKind.SkipClause,
             SyntaxKind.TakeClause              : Return DirectCast(syntax, PartitionClauseSyntax).SkipOrTakeKeyword
        Case SyntaxKind.GroupByClause           : Return DirectCast(syntax, GroupByClauseSyntax).GroupKeyword
        Case SyntaxKind.GroupJoinClause         : Return DirectCast(syntax, GroupJoinClauseSyntax).GroupKeyword
        Case SyntaxKind.SimpleJoinClause        : Return DirectCast(syntax, SimpleJoinClauseSyntax).JoinKeyword
        Case SyntaxKind.OrderByClause           : Return DirectCast(syntax, OrderByClauseSyntax).OrderKeyword
        Case SyntaxKind.SelectClause            : Return DirectCast(syntax, SelectClauseSyntax).SelectKeyword
        Case Else
    ...
    Friend Function EnclosingStructuredTrivia(node As VisualBasicSyntaxNode) As StructuredTriviaSyntax
      While node IsNot Nothing
        If node.IsStructuredTrivia Then Return DirectCast(node, StructuredTriviaSyntax)
        node = node.Parent
      End While
      Return Nothing
    End Function
