    ...
    Public Function MoveNext() As Boolean
      While _count > 0
        ...
        If diags IsNot Nothing AndAlso diagIndex < diags.Length - 1 Then
          ...
          If inDocumentationComment Then di = ErrorFactory.ErrorInfo(ERRID.WRN_XMLDocParseError1, di)
          ...
          If Not node.IsToken Then position += node.GetLeadingTriviaWidth()
...
tryAgain:
        If slotIndex < node.SlotCount - 1 Then
          ...
          If child Is Nothing Then GoTo tryAgain
        ...
        Else
          If node.SlotCount = 0 Then _position += node.Width
          Pop()
        End If
      End While
      Return False
    ...
    Private Sub PushToken(token As InternalSyntax.SyntaxToken, inDocumentationComment As Boolean)
      ...
      If trailing IsNot Nothing Then Push(trailing, inDocumentationComment)

      PushNode(token, inDocumentationComment)

      Dim leading = token.GetLeadingTrivia
      If leading IsNot Nothing Then Push(leading, inDocumentationComment)
    End Sub

    Private Sub PushNode(node As GreenNode, inDocumentationComment As Boolean)
      If _count >= _stack.Length Then
        Dim tmp = New NodeIteration((Me._stack.Length * 2) - 1) {}
