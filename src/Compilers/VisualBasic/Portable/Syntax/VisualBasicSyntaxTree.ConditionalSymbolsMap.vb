...
#End Region
      ...
      Friend Function GetPreprocessingSymbolInfo(conditionalSymbolName As String, node As IdentifierNameSyntax) As VisualBasicPreprocessingSymbolInfo
        ...
        If constValue Is Nothing Then Return VisualBasicPreprocessingSymbolInfo.None
      ...
      Private Function GetPreprocessorSymbolValue(conditionalSymbolName As String, node As SyntaxNodeOrToken) As InternalSyntax.CConst
        ...
        If _conditionalsMap.TryGetValue(conditionalSymbolName, values) Then
          ...
          For Each valueTuple In values
            If valueTuple.Item2 < position Then Return valueTuple.Item1
          Next
        End If
      ...
      Friend Function IsConditionalSymbolDefined(conditionalSymbolName As String, node As SyntaxNodeOrToken) As Boolean
        If conditionalSymbolName Is Nothing Then Return False
        Dim constValue As InternalSyntax.CConst = GetPreprocessorSymbolValue(conditionalSymbolName, node)
        If constValue IsNot Nothing AndAlso Not constValue.IsBad Then
          Select Case constValue.SpecialType
            Case SpecialType.System_Boolean   : Return DirectCast(constValue, InternalSyntax.CConst(Of Boolean)).Value
            Case SpecialType.System_Byte      : Return DirectCast(constValue, InternalSyntax.CConst(Of Byte)).Value <> 0
            Case SpecialType.System_Int16     : Return DirectCast(constValue, InternalSyntax.CConst(Of Int16)).Value <> 0
            Case SpecialType.System_Int32     : Return DirectCast(constValue, InternalSyntax.CConst(Of Int32)).Value <> 0
            Case SpecialType.System_Int64     : Return DirectCast(constValue, InternalSyntax.CConst(Of Int64)).Value <> 0
            Case SpecialType.System_SByte     : Return DirectCast(constValue, InternalSyntax.CConst(Of SByte)).Value <> 0
            Case SpecialType.System_UInt16    : Return DirectCast(constValue, InternalSyntax.CConst(Of UInt16)).Value <> 0
            Case SpecialType.System_UInt32    : Return DirectCast(constValue, InternalSyntax.CConst(Of UInt32)).Value <> 0
            Case SpecialType.System_UInt64    : Return DirectCast(constValue, InternalSyntax.CConst(Of UInt64)).Value <> 0
