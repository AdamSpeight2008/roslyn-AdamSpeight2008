...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  ''' <summary> The parsed representation of a Visual Basic source document. </summary>
  Partial Public MustInherit Class VisualBasicSyntaxTree
    Inherits SyntaxTree

    ''' <summary> The options used by the parser to produce the syntax tree. </summary>
    Public MustOverride Shadows ReadOnly Property Options As VisualBasicParseOptions

    ''' <summary> Returns True for MyTemplate automatically added by compiler. </summary>
    ...
    ''' <summary> Gets the root node of the syntax tree asynchronously. </summary>
    ''' <remarks>
    ...
    ''' <summary> Gets the root node of the syntax tree if it is already available. </summary>
    Public MustOverride Shadows Function TryGetRoot(ByRef root As VisualBasicSyntaxNode) As Boolean

    ''' <summary> Gets the root of the syntax tree statically typed as <see cref="CompilationUnitSyntax"/>. </summary>
    ''' <remarks> Ensure that <see cref="SyntaxTree.HasCompilationUnitRoot"/> is true for this tree prior to invoking this method. </remarks>
    ...
    ''' <summary> Creates a new syntax based off this tree using a new source text. </summary>
    ''' <remarks>
    ...
    Public Overrides Function WithChangedText(newText As SourceText) As SyntaxTree
      ...
      If TryGetText(oldText) Then Return WithChanges(newText, newText.GetChangeRanges(oldText).ToArray())
    ...
    Public Shared Function ParseText(
                                      text As String,
                             Optional options As VisualBasicParseOptions = Nothing,
                             Optional path As String = "",
                             Optional encoding As Encoding = Nothing,
    ...
    Friend Overloads Function GetDiagnostics(node As InternalSyntax.VisualBasicSyntaxNode, position As Integer, InDocumentationComment As Boolean) As IEnumerable(Of Diagnostic)
      ...
      If node.ContainsDiagnostics Then Return EnumerateDiagnostics(node, position, InDocumentationComment)
    ...
    Private Function InDocumentationComment(node As SyntaxNode) As Boolean
      ...
      While node IsNot Nothing AndAlso Not SyntaxFacts.IsXmlSyntax(node.Kind())
        foundXml = True
        node = node.Parent
      End While
    ...
    Private Function InDocumentationComment(node As SyntaxNodeOrToken) As Boolean
      If node.IsToken Then Return InDocumentationComment(node.AsToken)
    ...
    ''' <summary> Gets a location for the specified text <paramref name="span"/>. </summary>
    ...
    ''' <summary> Determines if two trees are the same, disregarding trivia differences. </summary>
    ...
    ''' <summary> Gets a list of text changes that when applied to the old tree produce this tree. </summary>
    ''' <param name="oldTree">The old tree. Cannot be <c>Nothing</c>.</param>
...
#Region "SyntaxTree"
    ...
    Protected Overrides Function TryGetRootCore(ByRef root As SyntaxNode) As Boolean
      Dim node As VisualBasicSyntaxNode = Nothing
      Dim ok = TryGetRoot(node)
      root = If(ok, root, Nothing)
      Return ok
    End Function
...
#Region "Preprocessor Symbols"
    ...
    Friend Function IsAnyPreprocessorSymbolDefined(conditionalSymbolNames As IEnumerable(Of String), atNode As SyntaxNodeOrToken) As Boolean
      ...
      If conditionalSymbolsMap Is Nothing Then Return False

      For Each conditionalSymbolName As String In conditionalSymbolNames
        If conditionalSymbolsMap.IsConditionalSymbolDefined(conditionalSymbolName, atNode) Then Return True
      Next
