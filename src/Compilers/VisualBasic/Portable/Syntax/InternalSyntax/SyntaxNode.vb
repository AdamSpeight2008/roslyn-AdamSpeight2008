...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax.InternalSyntax
  ...
  Partial Friend Class VisualBasicSyntaxNode
    ...
    Protected Sub SetFactoryContext(
    ...
    Friend Function MatchesFactoryContext(
                                           context As ISyntaxFactoryContext
                                         ) As Boolean
...
#End Region
   ...
   Friend Sub New(
                   ...
                   diagnostics As DiagnosticInfo(),
                   annotations As SyntaxAnnotation(), 
                   fullWidth   As Integer
                 )
     MyBase.New(CType(kind, UInt16), diagnostics, annotations, fullWidth)
     GreenStats.NoteGreen(Me)
   End Sub

   ''' <summary> Get all syntax errors associated with this node, or any child nodes, grand-child nodes, etc. The errors are not in order. </summary>
   ...
   Friend Overridable Sub AddSyntaxErrors(
                                           accumulatedErrors As List(Of DiagnosticInfo)
                                         )
     ...
     For i As Integer = 0 To cnt - 1
       Dim child = GetSlot(i)
       If child IsNot Nothing AndAlso child.ContainsDiagnostics Then DirectCast(child, VisualBasicSyntaxNode).AddSyntaxErrors(accumulatedErrors)
     Next
   End Sub
   ...
   Friend Overloads Shared Function IsEquivalentTo(
                                                    left  As VisualBasicSyntaxNode,
                                                    right As VisualBasicSyntaxNode
                                                  ) As Boolean
   ...
   Private Shared ReadOnly s_structuresTable As New ConditionalWeakTable(Of SyntaxNode, Dictionary(Of CodeAnalysis.SyntaxTrivia, SyntaxNode))

   Public Overrides Function GetStructure(
                                           trivia As CodeAnalysis.SyntaxTrivia
                                         ) As SyntaxNode
     If Not trivia.HasStructure Then Return Nothing
     Dim parent = trivia.Token.Parent
     If parent Is Nothing Then Return Syntax.StructuredTriviaSyntax.Create(trivia)
   ...
   Public Overrides Function CreateSeparator(Of TNode As SyntaxNode)(
                                                                      element As SyntaxNode
                                                                    ) As CodeAnalysis.SyntaxToken
     Dim separatorKind As SyntaxKind = SyntaxKind.CommaToken
     If element.Kind = SyntaxKind.JoinCondition Then separatorKind = SyntaxKind.AndKeyword
