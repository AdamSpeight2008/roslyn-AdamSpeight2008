...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax.InternalSyntax
  ...
  Friend Module SyntaxExtensions
    ...
    <Extension>
    Public Function WithAnnotations(Of TNode As VisualBasicSyntaxNode)(
                                                                        node            As TNode,
                                                             ParamArray annotations()   As SyntaxAnnotation
                                                                      ) As TNode
    ...
    <Extension>
    Public Function WithAdditionalAnnotations(Of TNode As VisualBasicSyntaxNode)(
                                                                                  node          As TNode,
                                                                       ParamArray annotations() As SyntaxAnnotation
                                                                                ) As TNode
      If annotations Is Nothing Then Throw New ArgumentNullException(NameOf(annotations))
    ...
    <Extension>
    Public Function WithoutAnnotations(Of TNode As VisualBasicSyntaxNode)(
                                                                           node                 As TNode,
                                                                ParamArray removalAnnotations() As SyntaxAnnotation
                                                                         ) As TNode
      ...
      For Each candidate In annotations
        If Array.IndexOf(removalAnnotations, candidate) < 0 Then  newAnnotations.Add(candidate)
      Next
      Return CType(node.SetAnnotations(newAnnotations.ToArrayAndFree()), TNode)
    End Function

    <Extension>
    Public Function WithAdditionalDiagnostics(Of TNode As GreenNode)(
                                                                      node          As TNode,
                                                           ParamArray diagnostics   As DiagnosticInfo()
                                                                    ) As TNode
      Dim current As DiagnosticInfo() = node.GetDiagnostics
      If current IsNot Nothing Then Return DirectCast(node.SetDiagnostics(current.Concat(diagnostics).ToArray()), TNode)
      Return node.WithDiagnostics(diagnostics)
    End Function

    <Extension>
    Public Function WithDiagnostics(Of TNode As GreenNode)(
                                                            node        As TNode,
                                                 ParamArray diagnostics As DiagnosticInfo()
                                                          ) As TNode
    ...
    <Extension>
    Public Function WithoutDiagnostics(Of TNode As VisualBasicSyntaxNode)(
                                                                           node As TNode
                                                                         ) As TNode
      Dim current As DiagnosticInfo() = node.GetDiagnostics
      If ((current Is Nothing) OrElse (current.Length = 0)) Then Return node
      Return DirectCast(node.SetDiagnostics(Nothing), TNode)
    End Function

    <Extension>
    Public Function LastTriviaIfAny(
                                     node As VisualBasicSyntaxNode
                                   ) As VisualBasicSyntaxNode
      Dim trailingTriviaNode = node.GetTrailingTrivia()
      If trailingTriviaNode Is Nothing Then Return Nothing
      Return New CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)(trailingTriviaNode).Last
    End Function

    <Extension>
    Public Function EndsWithEndOfLineOrColonTrivia(
                                                    node As VisualBasicSyntaxNode
                                                  ) As Boolean
      Dim trailingTrivia = node.LastTriviaIfAny()
      Return trailingTrivia IsNot Nothing AndAlso trailingTrivia.Kind.IsAny(SyntaxKind.EndOfLineTrivia, SyntaxKind.ColonTrivia)
    End Function
  End Module

  Friend Module SyntaxNodeExtensions

    Private Function IsMissingToken(
...
#Region "AddLeading"
    ...
    <Extension>
    Private Function AddLeadingTrivia(Of TSyntax As VisualBasicSyntaxNode)(
                                                                            node    As TSyntax,
                                                                            trivia  As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
                                                                          ) As TSyntax
      If node Is Nothing Then Throw New ArgumentNullException(NameOf(node))
      If Not trivia.Any Then Return node
    ...
    <Extension>
    Friend Function AddLeadingSyntax(Of TSyntax As VisualBasicSyntaxNode)(
                                                                           node         As TSyntax,
                                                                           unexpected   As CoreInternalSyntax.SyntaxList(Of GreenNode),
                                                                           errorId      As ERRID
                                                                         ) As TSyntax
      Dim diagnostic = ErrorFactory.ErrorInfo(errorId)
      If unexpected.Node Is Nothing Then Return DirectCast(node.AddError(diagnostic), TSyntax)
    ...
    Friend Function AddLeadingSyntax(Of TSyntax As VisualBasicSyntaxNode)(
                                                                           node         As TSyntax,
                                                                           unexpected   As CoreInternalSyntax.SyntaxList(Of SyntaxToken)
                                                                         ) As TSyntax
    ...
    Friend Function AddLeadingSyntax(Of TSyntax As VisualBasicSyntaxNode)(
                                                                           node         As TSyntax,
                                                                           unexpected   As GreenNode,
                                                                           errorId      As ERRID
                                                                         ) As TSyntax
      Dim diagnostic = ErrorFactory.ErrorInfo(errorId)
      If unexpected Is Nothing Then Return DirectCast(node.AddError(diagnostic), TSyntax)
    ...
    Friend Function AddLeadingSyntax(Of TSyntax As VisualBasicSyntaxNode)(
      ...
      If unexpected Is Nothing Then Return node
      Dim trivia As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode) = CreateSkippedTrivia(unexpected,
                                                                                                  preserveDiagnostics:=True,
                                                                                                  addDiagnosticToFirstTokenOnly:=False,
...
#Region "AddTrailing"
    ...
    <Extension>
    Friend Function AddTrailingTrivia(Of TSyntax As GreenNode)(
                                                                node    As TSyntax,
                                                                trivia  As CoreInternalSyntax.SyntaxList(Of GreenNode)
                                                              ) As TSyntax
      If node Is Nothing Then Throw New ArgumentNullException(NameOf(node))
      Dim tk = TryCast(node, SyntaxToken)
      Dim result As TSyntax
    ...
    ' to the first token in the list.
    <Extension>
    Friend Function AddTrailingSyntax(Of TSyntax As VisualBasicSyntaxNode)(
                                                                            node        As TSyntax,
                                                                            unexpected  As CoreInternalSyntax.SyntaxList(Of SyntaxToken),
                                                                            errorId     As ERRID
                                                                          ) As TSyntax
    ...
    <Extension>
     ...
     ' a new diagnostic with the given "errorId".
     <Extension>
     Friend Function AddTrailingSyntax(Of TSyntax As GreenNode)(
                                                                 node       As TSyntax,
                                                                 unexpected As GreenNode,
                                                                 errorId    As ERRID
                                                               ) As TSyntax
     ...
     <Extension>
     Friend Function AddError(Of TSyntax As VisualBasicSyntaxNode)(
                                                                    node    As TSyntax,
                                                                    errorId As ERRID
                                                                  ) As TSyntax
...
#End Region
    ...
    Friend Function GetStartOfTrivia(
                                      trivia     As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode),
                                      indexOfEnd As Integer
                                    ) As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
      If indexOfEnd = 0 Then Return Nothing
      If indexOfEnd = trivia.Count Then Return trivia
    ...
    <Extension>
    Friend Function GetEndOfTrivia(
                                    trivia As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
                                  ) As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
    ...
    Friend Function GetEndOfTrivia(
      ...
      If indexOfEnd = 0 Then Return trivia
      If indexOfEnd = trivia.Count Then Return Nothing
      Dim builder = SyntaxListBuilder(Of VisualBasicSyntaxNode).Create()
    ...
    ''' </summary>
    Friend Function GetLengthOfCommonEnd(
                                          trivia1 As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode),
                                          trivia2 As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
                                        ) As Integer
...
#End If
    ...
    <Extension>
    Private Function GetIndexAfterLastSkippedToken(
                                                    trivia As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
                                                  ) As Integer
      Dim n = trivia.Count
      For i = n - 1 To 0 Step -1
        If trivia(i).Kind = SyntaxKind.SkippedTokensTrivia Then Return i + 1
      Next
      Return 0
    End Function
    ...
    ''' </summary>
    <Extension>
    Private Function GetIndexOfEndOfTrivia(
                                            trivia As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
                                          ) As Integer
      Dim n = trivia.Count
      ...
      Select Case trivia(i).Kind
             Case SyntaxKind.ColonTrivia   : Return i
             Case SyntaxKind.EndOfLineTrivia
                  ...
                  End Select
                  Return i
             Case SyntaxKind.LineContinuationTrivia,           SyntaxKind.IfDirectiveTrivia,
                  SyntaxKind.ElseIfDirectiveTrivia,            SyntaxKind.ElseDirectiveTrivia,
                  SyntaxKind.EndIfDirectiveTrivia,             SyntaxKind.RegionDirectiveTrivia,
                  SyntaxKind.EndRegionDirectiveTrivia,         SyntaxKind.ConstDirectiveTrivia,
                  SyntaxKind.ExternalSourceDirectiveTrivia,    SyntaxKind.EndExternalSourceDirectiveTrivia,
                  SyntaxKind.ExternalChecksumDirectiveTrivia,  SyntaxKind.EnableWarningDirectiveTrivia,
                  SyntaxKind.DisableWarningDirectiveTrivia,    SyntaxKind.ReferenceDirectiveTrivia,
...
#Region "Skipped trivia creation"
    ...
    Private Function TriviaListContainsStructuredTrivia(triviaList As GreenNode) As Boolean
      If triviaList Is Nothing Then             Return False
      ...
      For i = 0 To trivia.Count - 1
        Select Case trivia.ItemUntyped(i).RawKind
               Case SyntaxKind.XmlDocument,                        SyntaxKind.SkippedTokensTrivia,
                    SyntaxKind.IfDirectiveTrivia,                  SyntaxKind.ElseIfDirectiveTrivia,
                    ...
                    SyntaxKind.ConstDirectiveTrivia,               SyntaxKind.ExternalSourceDirectiveTrivia,
                    SyntaxKind.EndExternalSourceDirectiveTrivia,   SyntaxKind.ExternalChecksumDirectiveTrivia,
                    SyntaxKind.ReferenceDirectiveTrivia,           SyntaxKind.EnableWarningDirectiveTrivia,
                    SyntaxKind.DisableWarningDirectiveTrivia,      SyntaxKind.BadDirectiveTrivia
                    Return True
        End Select
      Next
      Return False
    ...
    Private Class SkippedTriviaBuilder
      ...
      Public Sub New(
                      preserveExistingDiagnostics       As Boolean,
      ...
      Public Sub AddToken(
        ...
        If Not _preserveExistingDiagnostics Then token = token.WithoutDiagnostics()
        ...
        If isFirst AndAlso _addDiagnosticsToFirstTokenOnly Then                    FinishInProgressTokens() ' implicitly adds the diagnostics.
      End Sub
      ...
      Public Function GetTriviaList() As CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)
        ...
        If _diagnosticsToAdd IsNot Nothing AndAlso _diagnosticsToAdd.Any() Then
          ...
          If _triviaListBuilder.Count > 0 Then _triviaListBuilder(_triviaListBuilder.Count - 1) = _triviaListBuilder(_triviaListBuilder.Count - 1).WithAdditionalDiagnostics(_diagnosticsToAdd.ToArray())
        End If
    ...
    Private Function CreateSkippedTrivia(
                                          node                          As GreenNode,
                                          preserveDiagnostics           As Boolean,
                                          addDiagnosticToFirstTokenOnly As Boolean,
                                          addDiagnostic                 As DiagnosticInfo
                                        ) As CoreInternalSyntax.SyntaxList(Of GreenNode)
      If node.RawKind = SyntaxKind.SkippedTokensTrivia Then
        ' already skipped trivia
        If addDiagnostic IsNot Nothing Then node = node.AddError(addDiagnostic)
        Return node
      End If
      ...
      If Not preserveDiagnostics Then diagnostics.Clear()
      If addDiagnostic IsNot Nothing Then                diagnostics.Add(addDiagnostic)
    ...
    Friend Sub CollectConstituentTokensAndDiagnostics(
      ...
      For i = 0 To this.SlotCount() - 1
        ...
        If green IsNot Nothing Then CollectConstituentTokensAndDiagnostics(green, tokenListBuilder, nonTokenDiagnostics)
      Next
    End Sub
...
#Region "Whitespace Containment"
    <Extension>
    Friend Function ContainsWhitespaceTrivia(
                                              this As GreenNode
                                            ) As Boolean
      ...
      For i = 0 To trivia.Count - 1
        ...
        If kind = SyntaxKind.WhitespaceTrivia OrElse kind = SyntaxKind.EndOfLineTrivia Then Return True
      Next
      Return False
    End Function
#End Region
    ...
    <Extension>
    Friend Function ContainsCommentTrivia(
                                           this As GreenNode
                                         ) As Boolean
      If this Is Nothing Then Return False
      Dim trivia = New CoreInternalSyntax.SyntaxList(Of VisualBasicSyntaxNode)(this)
      For i = 0 To trivia.Count - 1
        Dim kind = trivia.ItemUntyped(i).RawKind
        If kind = SyntaxKind.CommentTrivia Then Return True
      Next
      Return False
    End Function
    ...
    <Extension>
    Friend Function ExtractAnonymousTypeMemberName(
       ...
       If conditionalAccessStack IsNot Nothing Then                conditionalAccessStack.Free()
       Return result
    End Function
...
TryAgain:
      Select Case input.Kind
             ...
             Case SyntaxKind.InvocationExpression
                  ...
                  If target Is Nothing Then Exit Select
             ...
             Case SyntaxKind.ConditionalAccessExpression
                  ...
                  If conditionalAccessStack Is Nothing Then conditionalAccessStack = ArrayBuilder(Of ConditionalAccessExpressionSyntax).GetInstance()
    ...
    Private Sub ClearConditionalAccessStack(
                                             conditionalAccessStack As ArrayBuilder(Of ConditionalAccessExpressionSyntax)
                                           )
      If conditionalAccessStack IsNot Nothing Then conditionalAccessStack.Clear()
    End Sub

    Private Function PopAndGetConditionalAccessReceiver(
                                                         conditionalAccessStack As ArrayBuilder(Of ConditionalAccessExpressionSyntax)
                                                       ) As ExpressionSyntax
      If conditionalAccessStack Is Nothing OrElse conditionalAccessStack.Count = 0 Then                Return Nothing
    ...
    Friend Function IsExecutableStatementOrItsPart(node As VisualBasicSyntaxNode) As Boolean
      If TypeOf node Is ExecutableStatementSyntax Then                Return True
      ...
      Select Case node.Kind
             Case SyntaxKind.IfStatement,       SyntaxKind.ElseIfStatement,     SyntaxKind.ElseStatement,     SyntaxKind.WithStatement,
                  SyntaxKind.TryStatement,      SyntaxKind.CatchStatement,      SyntaxKind.FinallyStatement,  SyntaxKind.SyncLockStatement,
                  SyntaxKind.WhileStatement,    SyntaxKind.UsingStatement,      SyntaxKind.SelectStatement,   SyntaxKind.CaseStatement,
                  SyntaxKind.CaseElseStatement, SyntaxKind.SimpleDoStatement,   SyntaxKind.DoWhileStatement,  SyntaxKind.DoUntilStatement,
