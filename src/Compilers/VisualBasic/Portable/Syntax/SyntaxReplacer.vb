...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax
  ...
  Private Class Replacer(Of TNode As SyntaxNode)
    ...
    Public Overrides Function Visit(node As SyntaxNode) As SyntaxNode
      ...
      If node IsNot Nothing Then
        ...
        If _nodeSet.Contains(node) AndAlso _computeReplacementNode IsNot Nothing Then rewritten = _computeReplacementNode(DirectCast(node, TNode), DirectCast(rewritten, TNode))
      End If
      Return rewritten
    End Function
    ...
    Public Overrides Function VisitToken(token As SyntaxToken) As SyntaxToken
      ...
      If _tokenSet.Contains(token) AndAlso _computeReplacementToken IsNot Nothing Then rewritten = _computeReplacementToken(token, rewritten)
      Return rewritten
    End Function
  ...
  End Class
        ...
        If TypeOf _originalNode Is TNode Then
          ...
          If index >= 0 AndAlso index < list.Count Then
            Select Case _editKind
              Case ListEditKind.Replace         : Return list.ReplaceRange(DirectCast(_originalNode, TNode), _replacementNodes.Cast(Of TNode))
              Case ListEditKind.InsertBefore    : Return list.InsertRange(index, _replacementNodes.Cast(Of TNode))
              Case ListEditKind.InsertAfter     : Return list.InsertRange(index + 1, _replacementNodes.Cast(Of TNode))
            End Select
          End If
        End If
    ...
    Private Class TokenListEditor   :   Inherits BaseListEditor
      ...
      Public Overrides Function VisitList(list As SyntaxTokenList) As SyntaxTokenList
        ...
        If index >= 0 AndAlso index < list.Count Then
          Select Case _editKind
            Case ListEditKind.Replace       : Return list.ReplaceRange(_originalToken, _newTokens)
            Case ListEditKind.InsertBefore  : Return list.InsertRange(index, _newTokens)
            Case ListEditKind.InsertAfter   : Return list.InsertRange(index + 1, _newTokens)
          End Select
        End If
        Return MyBase.VisitList(list)
      End Function
    ...
    Private Class TriviaListEditor  :   Inherits BaseListEditor
      ...
      Public Overrides Function VisitList(list As SyntaxTriviaList) As SyntaxTriviaList
        ...
        If index >= 0 AndAlso index < list.Count Then
          Select Case _editKind
            Case ListEditKind.Replace       : Return list.ReplaceRange(_originalTrivia, _newTrivia)
            Case ListEditKind.InsertBefore  : Return list.InsertRange(index, _newTrivia)
            Case ListEditKind.InsertAfter   : Return list.InsertRange(index + 1, _newTrivia)
          End Select
        End If
        Return MyBase.VisitList(list)
      End Function
