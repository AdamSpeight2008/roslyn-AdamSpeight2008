...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax
  ...
  Friend Class SyntaxNormalizer
     ...
     ''' <remarks></remarks>
     Private Sub New(
                      consideredSpan    As TextSpan,
                      indentWhitespace  As String,
                      eolWhitespace     As String,
                      useElasticTrivia  As Boolean,
     ...
     Friend Shared Function Normalize(Of TNode As SyntaxNode)(
                                                               node             As TNode,
                                                               indentWhitespace As String,
                                                               eolWhitespace    As String,
                                                               useElasticTrivia As Boolean,
     ...
     Friend Shared Function Normalize(
     ...
     Private Function GetIndentation(count As Integer) As SyntaxTrivia
       ...
       _indentations = If(_indentations, ArrayBuilder(Of SyntaxTrivia).GetInstance(capacity))
     ...
     Public Overrides Function VisitToken(token As SyntaxToken) As SyntaxToken
       ...
       If token.Kind = SyntaxKind.None Then Return token
       Try
         ...
         If numLineBreaksBefore > 0 AndAlso IsLastTokenOnLine(_previousToken) Then numLineBreaksBefore -= 1
         ...
         ElseIf newToken.Kind = SyntaxKind.XmlTextLiteralToken Then
           If newToken.TrailingTrivia.Count = 0 AndAlso IsNewLineChar(newToken.ValueText.Last) Then _afterLineBreak = True
         End If
         Return newToken
       Finally
     ...
     Private Function LineBreaksBetween(currentToken As SyntaxToken, nextToken As SyntaxToken) As Integer
       ...
       If currentToken.Kind = SyntaxKind.None OrElse nextToken.Kind = SyntaxKind.None Then Return 0
       Dim numLineBreaks As Integer = 0
       If _lineBreaksAfterToken.TryGetValue(currentToken, numLineBreaks) Then Return numLineBreaks
       Return 0
     End Function
     ...
     Private Function GetIndentationDepth(trivia As SyntaxTrivia) As Integer
       If SyntaxFacts.IsPreprocessorDirective(trivia.Kind) Then Return 0
     ...
     Private Function NeedsLineBreakBetween(trivia As SyntaxTrivia, nextTrivia As SyntaxTrivia, isTrailingTrivia As Boolean) As Boolean
       If EndsInLineBreak(trivia) Then Return False
     ...
     Private Function NeedsLineBreakBefore(trivia As SyntaxTrivia) As Boolean
       Select Case trivia.Kind
         Case SyntaxKind.DocumentationCommentExteriorTrivia : Return True
       End Select 
       Return False
     End Function
     ...
     Private Function NeedsIndentAfterLineBreak(trivia As SyntaxTrivia) As Boolean
       Select Case trivia.Kind
         Case SyntaxKind.CommentTrivia,
              SyntaxKind.DocumentationCommentExteriorTrivia,
              SyntaxKind.DocumentationCommentTrivia             : Return True
       End Select
       Return False
     End Function

     Private Function NeedsSeparator(token As SyntaxToken, nextToken As SyntaxToken) As Boolean
       If token.Kind = SyntaxKind.EndOfFileToken Then Return False
       If token.Parent Is Nothing OrElse nextToken.Kind = SyntaxKind.None Then Return False
       If nextToken.Parent.Kind = SyntaxKind.SingleLineFunctionLambdaExpression Then Return True
       If nextToken.Kind = SyntaxKind.EndOfFileToken Then Return False
       ' +1 instead of + 1
       ...
       If TypeOf (token.Parent) Is BinaryExpressionSyntax OrElse TypeOf (nextToken.Parent) Is BinaryExpressionSyntax Then Return True
       ' (a instead of ( a
       If token.Kind = SyntaxKind.OpenParenToken Then Return False
       ' a) instead of a )
       If nextToken.Kind = SyntaxKind.CloseParenToken Then Return False
       ' m( instead of m (
       If token.Kind <> SyntaxKind.CommaToken AndAlso nextToken.Kind = SyntaxKind.OpenParenToken Then Return False
       ...
       ' <e/> instead of <e />
       If nextToken.Kind = SyntaxKind.SlashGreaterThanToken Then Return False

       ' <!--a--> instead of <!-- a -->
       If token.Kind = SyntaxKind.LessThanExclamationMinusMinusToken OrElse nextToken.Kind = SyntaxKind.MinusMinusGreaterThanToken Then Return False

       ' <?xml ?> instead of <? xml ?>
       If token.Kind = SyntaxKind.LessThanQuestionToken Then Return False

       ' <![CDATA[goo]]> instead of <![CDATA[ goo ]]>
       If token.Kind = SyntaxKind.BeginCDataToken OrElse nextToken.Kind = SyntaxKind.EndCDataToken Then Return False
       ...
       ' <x/>@a instead of <x/>@ a
       If token.Kind = SyntaxKind.AtToken AndAlso token.Parent.Kind = SyntaxKind.XmlAttributeAccessExpression Then Return False
       ...
       If nextToken.Kind = SyntaxKind.QuestionToken Then Return False

       ' #if instead of # if
       If token.Kind = SyntaxKind.HashToken AndAlso TypeOf token.Parent Is DirectiveTriviaSyntax Then Return False
       ...
       If token.Kind = SyntaxKind.XmlTextLiteralToken OrElse token.Kind = SyntaxKind.DocumentationCommentLineBreakToken Then Return False

       If token.Kind = SyntaxKind.DollarSignDoubleQuoteToken Then Return False

       If token.Kind = SyntaxKind.InterpolatedStringTextToken OrElse nextToken.Kind = SyntaxKind.InterpolatedStringTextToken Then Return False

       Return True
     End Function

     Private Function EndsInLineBreak(trivia As SyntaxTrivia) As Boolean
       If trivia.Kind = SyntaxKind.EndOfLineTrivia Then Return True
     ...
     Private Shared Function IsNewLineChar(ch As Char) As Boolean
       ...
       Return ch = vbLf OrElse ch = vbCr OrElse ch = "\u0085" OrElse ch = "\u2028" OrElse ch = "\u2029"
     End Function
     ...
     Private Function GetNextRelevantToken(token As SyntaxToken) As SyntaxToken
       Dim nextToken = token.GetNextToken(Function(t As SyntaxToken) t.Kind <> SyntaxKind.None, Function(t As SyntaxTrivia) False)
       If _consideredSpan.Contains(nextToken.FullSpan) Then Return nextToken
       Return Nothing
     End Function
     ...
     Private Sub AddLinebreaksAfterTokenIfNeeded(node As SyntaxToken, linebreaksAfterToken As Integer)
       If Not EndsWithColonSeparator(node) Then _lineBreaksAfterToken(node) = linebreaksAfterToken
     End Sub

     Private Function EndsWithColonSeparator(node As SyntaxToken) As Boolean
        Return node.HasTrailingTrivia AndAlso node.TrailingTrivia.Last.Kind = SyntaxKind.ColonTrivia
     End Function

     Private Sub MarkLastStatementIfNeeded(Of TNode As SyntaxNode)(list As SyntaxList(Of TNode))
       If list.Any Then _lastStatementsInBlocks.Add(list.Last)
     End Sub
     ...
     Public Overrides Function VisitMultiLineIfBlock(node As MultiLineIfBlockSyntax) As SyntaxNode
       ...
       If node.ElseBlock IsNot Nothing Then AddLinebreaksAfterTokenIfNeeded(previousNode.GetLastToken(), 1)
     ...
     Public Overrides Function VisitWhileBlock(node As WhileBlockSyntax) As SyntaxNode
       ...
       AddLinebreaksAfterElementsIfNeeded(node.Statements, 1, 1)
       MarkLastStatementIfNeeded(node.Statements)
       If Not _lastStatementsInBlocks.Contains(node) Then AddLinebreaksAfterTokenIfNeeded(node.EndWhileStatement.GetLastToken(), 2)
   ...
   End Function
    ...
    Public Overrides Function VisitEventStatement(node As EventStatementSyntax) As SyntaxNode
      ...
      ' only indent if this is a block
      If node.Parent IsNot Nothing AndAlso node.Parent.Kind = SyntaxKind.EventBlock Then _indentationDepth += 1
      Return result
    End Function
    ...
    Public Overrides Function VisitPropertyStatement(node As PropertyStatementSyntax) As SyntaxNode
      ...
      ' only indent if this is a block
      If node.Parent IsNot Nothing AndAlso node.Parent.Kind = SyntaxKind.PropertyBlock Then _indentationDepth += 1
      Return result
    End Function
    ...
    Public Overrides Function VisitNextStatement(node As NextStatementSyntax) As SyntaxNode
      ...
      If variableCount = 0 Then variableCount = 1
