...
#Region "ParseMethods"
    ...
    ''' <summary> Create a new syntax tree from a syntax node. </summary>
    ...
    ''' <summary> Produces a syntax tree by parsing the source text. </summary>
    ...
    ''' <summary> Parse the input for leading trivia. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseLeadingTrivia(
                                               text     As String,
    ...
    ''' <summary> Parse the input for trailing trivia. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseTrailingTrivia(
                                                text    As String,
                                       Optional offset  As Integer = 0
                                              ) As SyntaxTriviaList
      Dim s = New InternalSyntax.Scanner(MakeSourceText(text, offset), VisualBasicParseOptions.Default)
    ...
    ''' <summary> Parse one token. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    ''' <param name="startStatement">Scan using rules for the start of a statement</param>
    Public Shared Function ParseToken(
                                       text             As String,
                              Optional offset           As Integer = 0,
                              Optional startStatement   As Boolean = False
                                     ) As SyntaxToken
      Dim s = New InternalSyntax.Scanner(MakeSourceText(text, offset), VisualBasicParseOptions.Default)
      Using s
        Dim state = If(startStatement, InternalSyntax.ScannerState.VBAllowLeadingMultilineTrivia, InternalSyntax.ScannerState.VB)
        s.GetNextTokenInState(state)
        Return New SyntaxToken(Nothing, s.GetCurrentToken, 0, 0)
      End Using
    End Function

    ''' <summary> Parse tokens in the input. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    ''' <param name="initialTokenPosition">The position of the first token</param>
    Public Shared Iterator Function ParseTokens(
                                                 text                   As String,
                                        Optional offset                 As Integer = 0,
                                        Optional initialTokenPosition   As Integer = 0,
                                        Optional options                As VisualBasicParseOptions = Nothing
                                               ) As IEnumerable(Of SyntaxToken)
      ...
      Using parser = New InternalSyntax.Parser(MakeSourceText(text, offset), If(options, VisualBasicParseOptions.Default))
        ...
        Do
          ...
          state = If(curTk.Kind = SyntaxKind.StatementTerminatorToken, InternalSyntax.ScannerState.VBAllowLeadingMultilineTrivia, InternalSyntax.ScannerState.VB)
        Loop While curTk.Kind <> SyntaxKind.EndOfFileToken
      End Using
    End Function

    ''' <summary> Parse a name. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseName(
                                      text              As String,
                             Optional offset            As Integer = 0,
                             Optional consumeFullText   As Boolean = True
                                    ) As NameSyntax
    ...
    ''' <summary> Parse a type name. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseTypeName(
                                          text              As String,
                                 Optional offset            As Integer = 0,
                                 Optional consumeFullText   As Boolean = True
                                        ) As TypeSyntax
    ...
    ''' <summary> Parse an executable statement. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseExecutableStatement(
                                                     text            As String,
                                            Optional offset          As Integer = 0,
                                            Optional consumeFullText As Boolean = True
                                                   ) As StatementSyntax
    ...
    ''' <summary> Parse a compilation unit (a single source file). </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseCompilationUnit(
                                                 text       As String,
                                        Optional offset     As Integer = 0,
                                        Optional options    As VisualBasicParseOptions = Nothing
    ...
    ''' <summary> Parse a parameter list. </summary>
    ''' <param name="text">The input string</param>
    ''' <param name="offset">The starting offset in the string</param>
    Public Shared Function ParseParameterList(
                                               text             As String,
                                      Optional offset           As Integer = 0,
                                      Optional consumeFullText  As Boolean = True
    ...
    ''' <summary> Helper method for wrapping a string and offset in an SourceText. </summary>
    Friend Shared Function MakeSourceText(
                                           text     As String,
                                           offset   As Integer
                                         ) As SourceText
    ...
    ''' </summary>
    Friend Shared Function ParseDocCommentAttributeAsStandAloneEntity(
                                                                       text              As String,
                                                                       parentElementName As String
                                                                     ) As BaseXmlAttributeSyntax
      ...
      Using scanner As New InternalSyntax.Scanner(MakeSourceText(text, 0), VisualBasicParseOptions.Default) ' NOTE: Default options should be enough
        ...
        Dim xmlName = InternalSyntax.SyntaxFactory.XmlName(Nothing, InternalSyntax.SyntaxFactory.XmlNameToken(parentElementName, SyntaxKind.XmlNameToken, Nothing, Nothing))
...
#Region "TokenFactories"
    Public Shared Function IntegerLiteralToken(
                                                text        As String,
                                                base        As LiteralBase,
    ...
    Public Shared Function IntegerLiteralToken(
                                                leadingTrivia   As SyntaxTriviaList,
                                                text            As String,
                                                base            As LiteralBase,
                                                typeSuffix      As TypeCharacter,
                                                value           As ULong,
                                                trailingTrivia  As SyntaxTriviaList
                                              ) As SyntaxToken
    ...
    Public Shared Function Identifier(leadingTrivia As SyntaxTriviaList, text As String, isBracketed As Boolean, identifierText As String, typeCharacter As TypeCharacter, trailingTrivia As SyntaxTriviaList) As SyntaxToken
      If text Is Nothing Then Throw New ArgumentException(NameOf(text))
    ...
    ''' <summary> Create a bracketed identifier. </summary>
    ...
    Public Shared Function BracketedIdentifier(leadingTrivia As SyntaxTriviaList, text As String, trailingTrivia As SyntaxTriviaList) As SyntaxToken
      If text Is Nothing Then Throw New ArgumentException(NameOf(text))
      If MakeHalfWidthIdentifier(text.First) = "[" AndAlso MakeHalfWidthIdentifier(text.Last) = "]" Then Throw New ArgumentException(NameOf(text))
    ...
    Friend Shared Function MissingIdentifier() As SyntaxToken
      Return New SyntaxToken(Nothing, New InternalSyntax.SimpleIdentifierSyntax(SyntaxKind.IdentifierToken, Nothing, Nothing, "", ElasticMarker.UnderlyingNode, ElasticMarker.UnderlyingNode), 0, 0)
    End Function
    ...
    Friend Shared Function MissingIdentifier(
      ...
      Return New SyntaxToken(Nothing, New InternalSyntax.ComplexIdentifierSyntax(SyntaxKind.IdentifierToken, Nothing, Nothing, "", ElasticMarker.UnderlyingNode, ElasticMarker.UnderlyingNode, kind, False, "", TypeCharacter.None), 0, 0)
    End Function
    ...
    Friend Shared Function MissingKeyword(
      ...
      Return New SyntaxToken(Nothing, New InternalSyntax.KeywordSyntax(kind, "", ElasticMarker.UnderlyingNode, ElasticMarker.UnderlyingNode), 0, 0)
    End Function
    ...
    Friend Shared Function MissingPunctuation(
      ...
      Return New SyntaxToken(Nothing, New InternalSyntax.PunctuationSyntax(kind, "", ElasticMarker.UnderlyingNode, ElasticMarker.UnderlyingNode), 0, 0)
    End Function
    ...
    Friend Shared Function MissingToken(
      ...
      Select Case kind
             Case SyntaxKind.StatementTerminatorToken     : Return SyntaxFactory.Token(SyntaxKind.StatementTerminatorToken)
             Case SyntaxKind.EndOfFileToken               : Return SyntaxFactory.Token(SyntaxKind.EndOfFileToken)
             Case SyntaxKind.AddHandlerKeyword,
                  ...
                  SyntaxKind.OutKeyword                       : Return SyntaxFactory.MissingKeyword(kind)
             Case SyntaxKind.AggregateKeyword,
                  ...
                  SyntaxKind.WhereKeyword                         : Return SyntaxFactory.MissingIdentifier(kind) ' These are identifiers that have a contextual kind
             Case SyntaxKind.ExclamationToken,
                  ...
                  SyntaxKind.QuestionToken                        : Return SyntaxFactory.MissingPunctuation(kind)
             Case SyntaxKind.FloatingLiteralToken                 : Return SyntaxFactory.FloatingLiteralToken("", TypeCharacter.None, Nothing)
             Case SyntaxKind.DecimalLiteralToken                  : Return SyntaxFactory.DecimalLiteralToken("", TypeCharacter.None, Nothing)
             Case SyntaxKind.DateLiteralToken                     : Return SyntaxFactory.DateLiteralToken("", Nothing)
             Case SyntaxKind.XmlNameToken                         : Return SyntaxFactory.XmlNameToken("", SyntaxKind.XmlNameToken)
             Case SyntaxKind.XmlTextLiteralToken                  : Return SyntaxFactory.XmlTextLiteralToken("", "")
             Case SyntaxKind.SlashGreaterThanToken,
                  ...
                  SyntaxKind.EndCDataToken                        : Return SyntaxFactory.MissingPunctuation(kind)
    ...
    Public Shared Function BadToken(
                                     text As String
                                   ) As SyntaxToken
    ...
    Public Shared Function BadToken(
      ...
      If text Is Nothing Then Throw New ArgumentException(NameOf(text))
      Return New SyntaxToken(Nothing, New InternalSyntax.BadTokenSyntax(SyntaxKind.BadToken, InternalSyntax.SyntaxSubKind.None, Nothing, Nothing, text, leadingTrivia.Node, trailingTrivia.Node), 0, 0)
    End Function
...
#Region "ListFactories"
     ...
     ''' <summary> Creates an empty list of syntax nodes. </summary>
     ...
     ''' <summary> Creates a singleton list of syntax nodes. </summary>
     ''' <typeparam name="TNode">The specific type of the element nodes.</typeparam>
     ...
     ''' <summary> Creates a list of syntax nodes. </summary>
     ''' <typeparam name="TNode">The specific type of the element nodes.</typeparam>
     ...
     ''' <summary> Creates an empty list of tokens. </summary>
     Public Shared Function TokenList() As SyntaxTokenList
       Return New SyntaxTokenList()
     End Function
     
     ''' <summary> Creates a singleton list of tokens. </summary>
     ...
     ''' <summary> Creates a list of tokens. </summary>
     ...
     ''' <summary> Creates an empty list of trivia. </summary>
     Public Shared Function TriviaList() As SyntaxTriviaList
       Return New SyntaxTriviaList()
     End Function
     
     ''' <summary> Creates a singleton list of trivia. </summary>
     ...
     Public Shared Function SeparatedList(Of TNode As SyntaxNode)(nodes As IEnumerable(Of TNode), separators As IEnumerable(Of SyntaxToken)) As SeparatedSyntaxList(Of TNode)
       If nodes IsNot Nothing Then
         ...
         If nodeEnum.MoveNext() Then
           builder.Add(nodeEnum.Current)
           If nodeEnum.MoveNext() Then Throw New ArgumentException()
         End If
         Return builder.ToList()
     ...
     Public Shared Function SeparatedList(Of TNode As SyntaxNode)(nodesAndTokens As SyntaxNodeOrTokenList) As SeparatedSyntaxList(Of TNode)
       If Not HasSeparatedNodeTokenPattern(nodesAndTokens) Then Throw New ArgumentException(CodeAnalysisResources.NodeOrTokenOutOfSequence)
       If Not NodesAreCorrectType(Of TNode)(nodesAndTokens) Then Throw New ArgumentException(CodeAnalysisResources.UnexpectedTypeOfNodeInList)
     ...
     Private Shared Function NodesAreCorrectType(Of TNode)(list As SyntaxNodeOrTokenList) As Boolean
       ...
       For i = 0 To n - 1
         Dim element = list(i)
         If element.IsNode AndAlso Not (TypeOf element.AsNode() Is TNode) Then Return False
       Next
       Return True
     End Function
     ...
     Private Shared Function HasSeparatedNodeTokenPattern(list As SyntaxNodeOrTokenList) As Boolean
       For i = 0 To list.Count - 1
         Dim element = list(i)
         If element.IsToken = ((i And 1) = 0) Then Return False
       Next
       Return True
     End Function
     
     ''' <summary> Creates an empty <see cref="SyntaxNodeOrTokenList"/>. </summary>
     Public Shared Function NodeOrTokenList() As SyntaxNodeOrTokenList
       Return Nothing
     End Function
     
     ''' <summary> Creates a <see cref="SyntaxNodeOrTokenList"/> from a sequence of nodes and tokens. </summary>
     ...
     ''' <summary> Creates a <see cref="SyntaxNodeOrTokenList"/> from one or more nodes and tokens. </summary>
