...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Partial Public Class SyntaxFacts
    ...
    ''' <summary> Determine if the token instance represents a syntax trivia such as comment, whitespace, etc... </summary>
    Public Shared Function IsTrivia(this As SyntaxKind) As Boolean
      Return IsSyntaxTrivia(this)
    End Function

    ''' <summary> Get all reserved and contextual keywords. </summary>
    ...
    ''' <summary> Helper to check whether the token is a predefined type. </summary>
    ...
    ''' <summary> Helper to check whether the token is a predefined type OR Variant keyword. </summary>
    ...
    ''' <summary> Returns true if the node is the operand of an AddressOf expression. </summary>
    ...
    Public Shared Function IsInTypeOnlyContext(node As ExpressionSyntax) As Boolean
      ...
      If Not (TypeOf node Is TypeSyntax) Then Return False ' Only nodes deriving from TypeSyntax could possible be in type context.

      Dim parent As VisualBasicSyntaxNode = node.Parent
      If parent Is Nothing Then Return False
      Select Case parent.Kind
             Case SyntaxKind.SimpleAsClause,
                  SyntaxKind.AsNewClause                : Return DirectCast(parent, AsClauseSyntax).Type Is node
             Case SyntaxKind.GetTypeExpression          : Return DirectCast(parent, GetTypeExpressionSyntax).Type Is node
             Case SyntaxKind.TypeOfIsExpression,
                   SyntaxKind.TypeOfIsNotExpression     : Return DirectCast(parent, TypeOfExpressionSyntax).Type Is node
             Case SyntaxKind.CTypeExpression,
                  SyntaxKind.DirectCastExpression,
                  SyntaxKind.TryCastExpression          : Return DirectCast(parent, CastExpressionSyntax).Type Is node
             Case SyntaxKind.TypeArgumentList           : Return True  ' all non-token children are types
             Case SyntaxKind.InheritsStatement,
                  SyntaxKind.ImplementsStatement        : Return True  ' all non-token children are types
             Case SyntaxKind.TypeConstraint             : Return True ' all non-token children are types
             Case SyntaxKind.Attribute                  : Return DirectCast(parent, AttributeSyntax).Name Is node
             Case SyntaxKind.ObjectCreationExpression   : Return DirectCast(parent, ObjectCreationExpressionSyntax).Type Is node
             Case SyntaxKind.ArrayCreationExpression    : Return DirectCast(parent, ArrayCreationExpressionSyntax).Type Is node
             Case SyntaxKind.ArrayType                  : Return DirectCast(parent, ArrayTypeSyntax).ElementType Is node
             Case SyntaxKind.NullableType               : Return DirectCast(parent, NullableTypeSyntax).ElementType Is node
             Case SyntaxKind.QualifiedName               
                  Dim parentQualName = DirectCast(parent, QualifiedNameSyntax)
                  If parentQualName.Parent IsNot Nothing AndAlso parentQualName.Parent.Kind = SyntaxKind.ImplementsClause Then Return parentQualName.Left Is node
                  Return parentQualName.Right Is node
             Case SyntaxKind.TypedTupleElement          : Return DirectCast(parent, TypedTupleElementSyntax).Type Is node
          End Select   
      Return False
    End Function
    ...
    Friend Shared Function IsHandlesEvent(node As SyntaxNode) As Boolean
      Debug.Assert(node IsNot Nothing)
      Dim parent = node.Parent
      Return (parent IsNot Nothing) AndAlso (parent.IsKind(SyntaxKind.HandlesClauseItem)) AndAlso (TypeOf node Is IdentifierNameSyntax)
    End Function
    ...
    Friend Shared Function IsHandlesProperty(node As SyntaxNode) As Boolean
      Debug.Assert(node IsNot Nothing)
      Dim parent = node.Parent
      Return parent IsNot Nothing AndAlso parent.IsKind(SyntaxKind.WithEventsPropertyEventContainer) AndAlso TypeOf node Is IdentifierNameSyntax
    End Function
    ...
    ' Is this node in a place where is must bind to either a namespace or a type.
     Public Shared Function IsInNamespaceOrTypeContext(node As SyntaxNode) As Boolean
       ...
       If TypeOf node IsNot TypeSyntax Then Return False ' Only nodes deriving from TypeSyntax could possible be in type or namespace context.
       Dim parent = node.Parent
       If parent IsNot Nothing Then
         Select Case parent.Kind()
                Case SyntaxKind.SimpleImportsClause : Return DirectCast(parent, SimpleImportsClauseSyntax).Name Is node
                Case SyntaxKind.NamespaceStatement  : Return DirectCast(parent, NamespaceStatementSyntax).Name Is node
                Case SyntaxKind.QualifiedName
                     Dim parentQualName = DirectCast(parent, QualifiedNameSyntax)
                     If Not (parentQualName.Parent IsNot Nothing AndAlso parentQualName.Parent.Kind = SyntaxKind.ImplementsClause) Then Return DirectCast(parent, QualifiedNameSyntax).Left Is node
         End Select
       End If
       Dim expressionNode = TryCast(node, ExpressionSyntax)
       If expressionNode IsNot Nothing Then Return IsInTypeOnlyContext(expressionNode)
       Return False
     End Function
     ...
     Friend Shared Function InSpanOrEffectiveTrailingOfNode(node As SyntaxNode, position As Integer) As Boolean
       Dim span = node.Span
       If span.Contains(position) Then Return True
       If position >= span.End AndAlso position < node.FullSpan.End Then
         ' Position is in the trailing trivia of node. Check for newline or :.
          ...
          For Each trivia In trailingTrivia
            If trivia.Kind = SyntaxKind.EndOfLineTrivia OrElse trivia.Kind = SyntaxKind.ColonTrivia Then Exit For
            If trivia.FullSpan.Contains(position) Then Return True
          Next
     ...
     End Function
      ...
      ''' <summary> Determines if possibleLambda is a lambda expression and position is in the interior. </summary>
      ...
      Friend Shared Function InBlockInterior(
      ...
      Friend Shared Function IsBlockStatement(
                                               possibleBlock   As SyntaxNode,
      ...
      Friend Shared Function BeginOfBlockStatementIfAny(node As SyntaxNode) As SyntaxNode
        ...
        If IsBlockStatement(node, beginStatement, beginTerminator, body, endStatement) Then
          If beginStatement IsNot Nothing Then Return beginStatement
        End If
        Return node
      End Function
      ...
      Public Shared Function GetText(accessibility As Accessibility) As String
        Select Case accessibility
               Case Accessibility.Friend                : Return GetText(SyntaxKind.FriendKeyword)
               Case Accessibility.NotApplicable         : Return String.Empty
               Case Accessibility.Private               : Return GetText(SyntaxKind.PrivateKeyword)
               Case Accessibility.Protected             : Return GetText(SyntaxKind.ProtectedKeyword)
               Case Accessibility.ProtectedAndFriend    : Return GetText(SyntaxKind.PrivateKeyword) + " " + GetText(SyntaxKind.ProtectedKeyword)
               Case Accessibility.ProtectedOrFriend     : Return GetText(SyntaxKind.ProtectedKeyword) + " " + GetText(SyntaxKind.FriendKeyword)
               Case Accessibility.Public                : Return GetText(SyntaxKind.PublicKeyword)
               Case Else
      ...
      Public Shared Function GetUnaryExpression(token As SyntaxKind) As SyntaxKind
        Select Case token
               ...
               Case SyntaxKind.NotKeyword       : Return SyntaxKind.NotExpression
               Case SyntaxKind.AddressOfKeyword : Return SyntaxKind.AddressOfExpression
      ...
      Public Shared Function IsName(kind As SyntaxKind) As Boolean
        Return kind.IsAny(SyntaxKind.IdentifierName, SyntaxKind.GenericName, SyntaxKind.QualifiedName, SyntaxKind.GlobalName)
      End Function

      Public Shared Function IsNamespaceMemberDeclaration(kind As SyntaxKind) As Boolean
        Return kind.IsAny(SyntaxKind.ClassStatement,  SyntaxKind.InterfaceStatement, SyntaxKind.StructureStatement, SyntaxKind.EnumStatement,
                          SyntaxKind.ModuleStatement, SyntaxKind.NamespaceStatement, SyntaxKind.DelegateFunctionStatement, SyntaxKind.DelegateSubStatement)
      End Function
      ...
      Public Shared Function VarianceKindFromToken(token As SyntaxToken) As VarianceKind
        Select Case token.Kind
               ...
               Case Else                    : Return VarianceKind.None
        End Select
      End Function
        ...
        Do While nextNode IsNot Nothing
          Select Case nextNode.Kind()
                 Case SyntaxKind.IdentifierName,
                      SyntaxKind.QualifiedName      : nextNode = nextNode.Parent
                 Case SyntaxKind.Attribute
                      Dim attribute = DirectCast(nextNode, AttributeSyntax)
                      If attribute.Name Is node Then Return True
      ...
      Public Shared Function IsNamedArgumentName(node As SyntaxNode) As Boolean
        If node.Kind <> SyntaxKind.IdentifierName Then Return False
        Dim parent1 = TryCast(node.Parent, NameColonEqualsSyntax)
        If parent1 Is Nothing Then Return False

        Dim parent2 = parent1.Parent.Parent
        If parent2 Is Nothing OrElse Not parent2.IsKind(SyntaxKind.ArgumentList) Then Return False

        Dim parent3 = parent2.Parent
        If parent3 Is Nothing Then Return False
      ...
      Public Shared Function GetBlockName(kind As SyntaxKind) As String
        Select Case kind
               Case SyntaxKind.CaseBlock           : Return "Case"
               Case SyntaxKind.SimpleDoLoopBlock,
                    SyntaxKind.DoWhileLoopBlock,
                    SyntaxKind.DoUntilLoopBlock,
                    SyntaxKind.DoLoopWhileBlock,
                    SyntaxKind.DoLoopUntilBlock    : Return "Do Loop"
               Case SyntaxKind.WhileBlock          : Return "While"
               Case SyntaxKind.WithBlock           : Return "With"
               Case SyntaxKind.SyncLockBlock       : Return "SyncLock"
               Case SyntaxKind.UsingBlock          : Return "Using"
               Case SyntaxKind.ForBlock            : Return "For"
               Case SyntaxKind.ForEachBlock        : Return "For Each"
               Case SyntaxKind.SelectBlock         : Return "Select"
               Case SyntaxKind.MultiLineIfBlock    : Return "If"
               Case SyntaxKind.ElseIfBlock         : Return "Else If"
               Case SyntaxKind.ElseBlock           : Return "Else"
               Case SyntaxKind.TryBlock            : Return "Try"
               Case SyntaxKind.CatchBlock          : Return "Catch"
               Case SyntaxKind.FinallyBlock        : Return "Finally"
        End Select
    ...
    ''' </list>
    ''' </remarks>
    Public Shared Function AllowsTrailingImplicitLineContinuation(
      ...
      If kind = SyntaxKind.MinusToken OrElse kind = SyntaxKind.PlusToken Then Return TypeOf token.Parent Is BinaryExpressionSyntax
      ' Disallowed after xml namespace imports
      If kind = SyntaxKind.GreaterThanToken AndAlso parentKind = SyntaxKind.XmlNamespaceImportsClause Then Return False
      ...
      Select Case kind
             ...
             Case SyntaxKind.OpenBraceToken
                   Return parentKind <> SyntaxKind.Interpolation
             ...
             Case SyntaxKind.DotToken
                   Select Case parentKind
                       Case  SyntaxKind.NamedFieldInitializer           : Return False ' bug # 12903  when dot is directly under named field initializer, (_) is needed between "." and member name
                       Case  SyntaxKind.SimpleMemberAccessExpression    : Return CType(token.Parent, MemberAccessExpressionSyntax).Expression IsNot Nothing OrElse token.Parent.Parent.Kind = SyntaxKind.NamedFieldInitializer
                       ...
                       Case  SyntaxKind.XmlElementAccessExpression,
                            ...
                            If CType(token.Parent, XmlMemberAccessExpressionSyntax).Base IsNot Nothing Then Return token.GetNextToken.Kind <> SyntaxKind.DotToken
                            Return False
                   End Select
                   Return True
             ...
             Case SyntaxKind.GroupKeyword  : Return parentKind <> SyntaxKind.GroupJoinClause
             ' You cannot break a line between the keywords of query operators that are made up of multiple keywords 
             ' (Order By, Group Join, Take While, and Skip While).
             Case SyntaxKind.SkipKeyword   : Return parentKind <> SyntaxKind.SkipWhileClause
             ' You cannot break a line between the keywords of query operators that are made up of multiple keywords 
             ' (Order By, Group Join, Take While, and Skip While).
             Case SyntaxKind.TakeKeyword   : Return parentKind <> SyntaxKind.TakeWhileClause
             ...
             Case SyntaxKind.EndCDataToken,
                  ...
                  While n IsNot Nothing
                    p = n
                    If p.EndPosition > token.EndPosition Then Return True
    ...
    ''' </list>
    ''' </remarks>
    Public Shared Function AllowsLeadingImplicitLineContinuation(
                                                                  token As SyntaxToken
                                                                ) As Boolean
      If token.Parent Is Nothing Then Throw New ArgumentException("'token' must be parented by a SyntaxNode.")
      Dim kind = token.Kind
             ...
             ' Order By, Select, Skip, Skip While, Take, Take While, Where, In, Into, On, Ascending, and Descending). 
              ...
              Case SyntaxKind.AggregateKeyword,
                   ...
                   While p IsNot Nothing
                     If TypeOf p Is QueryExpressionSyntax Then Return p.GetLocation.SourceSpan.Start < token.SpanStart 
                     p = p.Parent
                   End While
                   Return False
              Case Else
    ...
    Friend Shared Function IsTerminator(kind As SyntaxKind) As Boolean
      Return kind.IsAny(SyntaxKind.StatementTerminatorToken, SyntaxKind.ColonToken, SyntaxKind.EndOfFileToken)
    End Function

    Friend Shared Function IsWithinPreprocessorConditionalExpression(
                                                                      node As SyntaxNode
                                                                    ) As Boolean
      Debug.Assert(node IsNot Nothing)
      Dim parent = node.Parent
    ...
    ''' </summary>
    Public Shared Function IsReservedTupleElementName(
