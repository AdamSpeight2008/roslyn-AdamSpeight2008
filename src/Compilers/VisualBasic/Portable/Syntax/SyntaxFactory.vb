...
Namespace Microsoft.CodeAnalysis.VisualBasic
    ...
    Partial Public Class SyntaxFactory
      ''' <summary> A trivia with kind EndOfLineTrivia containing both the carriage return And line feed characters. </summary>
      ...
      ''' <summary> A trivia with kind EndOfLineTrivia containing a single line feed character. </summary>
      Public Shared ReadOnly Property LineFeed As SyntaxTrivia = CType(InternalSyntax.SyntaxFactory.LineFeed, SyntaxTrivia)

      ''' <summary> A trivia with kind EndOfLineTrivia containing a single carriage return character. </summary>
      ...
      ''' <summary>  A trivia with kind WhitespaceTrivia containing a single space character. </summary>
      Public Shared ReadOnly Property Space As SyntaxTrivia = CType(InternalSyntax.SyntaxFactory.Space, SyntaxTrivia)

      ''' <summary> A trivia with kind WhitespaceTrivia containing a single tab character. </summary>
      ...
      Public Shared Function Literal(leading As SyntaxTriviaList, text As String, value As Decimal, trailing As SyntaxTriviaList) As SyntaxToken
        Return CType(InternalSyntax.SyntaxFactory.DecimalLiteralToken(text,
                 If(text.EndsWith("M", StringComparison.OrdinalIgnoreCase), TypeCharacter.DecimalLiteral, TypeCharacter.None),
                 value, leading.Node, trailing.Node), SyntaxToken)
      End Function
      ...
      Public Shared Function Literal(leading As SyntaxTriviaList, text As String, value As String, trailing As SyntaxTriviaList) As SyntaxToken
        Return CType(InternalSyntax.SyntaxFactory.StringLiteralToken(text, value, leading.Node, trailing.Node), SyntaxToken)
      End Function
      ...
      Public Shared Function Literal(leading As SyntaxTriviaList, text As String, value As Char, trailing As SyntaxTriviaList) As SyntaxToken
        Return CType(InternalSyntax.SyntaxFactory.CharacterLiteralToken(text, value, leading.Node, trailing.Node), SyntaxToken)
      End Function

      Public Shared Function TypeBlock(
                                        blockKind    As SyntaxKind,
                                        begin        As TypeStatementSyntax,
                               Optional [inherits]   As SyntaxList(Of InheritsStatementSyntax)   = Nothing,
                               Optional [implements] As SyntaxList(Of ImplementsStatementSyntax) = Nothing,
                               Optional members      As SyntaxList(Of StatementSyntax)           = Nothing, 
                               Optional [end]        As EndBlockStatementSyntax                  = Nothing
                                      ) As TypeBlockSyntax
      ...
      Public Shared Function TypeStatement(
                                   ...
                                   Optional modifiers         As SyntaxTokenList                    = Nothing,
                                   Optional keyword           As SyntaxToken                        = Nothing,
                                   Optional identifier        As SyntaxToken                        = Nothing,
                                   Optional typeParameterList As TypeParameterListSyntax            = Nothing
      ...
      Public Shared Function XmlParamElement(
                                              parameterName As String,
                                   ParamArray content       As XmlNodeSyntax()
                                            ) As XmlElementSyntax
      ...
      Public Shared Function XmlParamRefElement(
                                                 parameterName As String
      ...
      Private Shared Function XmlKeywordElement(
                                                 keyword As String
                                               ) As XmlEmptyElementSyntax
    ...
    Public Shared Function XmlPlaceholderElement(
                                       ParamArray content As XmlNodeSyntax()
                                                ) As XmlElementSyntax
    ...
    Public Shared Function XmlEmptyElement(
                                            localName As String
                                          ) As XmlEmptyElementSyntax
    ...
    Public Shared Function XmlElement(
                                       localName As String,
                                       content   As SyntaxList(Of XmlNodeSyntax)
                                     ) As XmlElementSyntax
    ...
    Public Shared Function XmlMultiLineElement(
                                                localName As String,
                                                content   As SyntaxList(Of XmlNodeSyntax)
                                              ) As XmlElementSyntax
    ...
    Public Shared Function XmlNewLine(
                                       text As String
                                     ) As XmlTextSyntax
    ...
    ''' </summary>
    ''' <param name="text">The raw text within the new line.</param>
    Public Shared Function XmlTextNewLine(
                                           text As String
                                         ) As SyntaxToken
    ...
    Public Shared Function XmlTextNewLine(
                                           text     As String,
                                           value    As String,
                                           leading  As SyntaxTriviaList,
                                           trailing As SyntaxTriviaList
                                         ) As SyntaxToken
    ...
    Public Shared Function XmlTextNewLine(
                                           text As String,
                                           continueXmlDocumentationComment As Boolean
                                         ) As SyntaxToken
      ...
      If continueXmlDocumentationComment Then token = token.WithTrailingTrivia(token.TrailingTrivia.Add(DocumentationCommentExteriorTrivia("''' ")))
      Return token
    End Function
    ...
    Public Shared Function XmlText(
                                    value As String
                                  ) As XmlTextSyntax
    ...
    Public Shared Function XmlText(
                         ParamArray textTokens As SyntaxToken()
                                  ) As XmlTextSyntax
    ...
    Public Shared Function XmlTextLiteral(
                                           value As String
                                         ) As SyntaxToken
    ...
    Public Shared Function XmlTextLiteral(
      ...
      Return New SyntaxToken(InternalSyntax.SyntaxFactory.XmlTextLiteralToken(text, value, ElasticMarker.UnderlyingNode, ElasticMarker.UnderlyingNode))
    End Function
    ...
    Private Shared Function XmlReplaceBracketTokens(
                                                     originalToken  As SyntaxToken,
                                                     rewrittenToken As SyntaxToken
                                                   ) As SyntaxToken
      If rewrittenToken.IsKind(SyntaxKind.LessThanToken) AndAlso
         String.Equals("<", rewrittenToken.Text, StringComparison.Ordinal) Then
      ...
      If rewrittenToken.IsKind(SyntaxKind.GreaterThanToken) AndAlso
         String.Equals(">", rewrittenToken.Text, StringComparison.Ordinal) Then
    ...
    Public Shared Function AreEquivalent(
                                          oldList As SyntaxTokenList,
                                          newList As SyntaxTokenList
                                        ) As Boolean
    ...
    ''' </param>
    Public Shared Function AreEquivalent(Of TNode As SyntaxNode)(
                                                                  oldList         As SyntaxList(Of TNode),
                                                                  newList         As SyntaxList(Of TNode),
                                                         Optional ignoreChildNode As Func(Of SyntaxKind, Boolean) = Nothing) As Boolean
    ...
    ''' </param>
    Public Shared Function AreEquivalent(Of TNode As SyntaxNode)(
                                                                  oldList         As SeparatedSyntaxList(Of TNode),
                                                                  newList         As SeparatedSyntaxList(Of TNode),
                                                         Optional ignoreChildNode As Func(Of SyntaxKind, Boolean) = Nothing
                                                                ) As Boolean
      Return SyntaxEquivalence.AreEquivalent(oldList.Node, newList.Node, ignoreChildNode, topLevel:=False)
    End Function
    ...
    Private Shared Function IsPartOfLinqQueryNotFollowedByNewLine(
                                                                   token          As SyntaxToken,
                                                                   statementNode  As SyntaxNode,
                                                                   endOfFileToken As SyntaxToken
                                                                 ) As Boolean
      ' Checking if the submission ends with a new line.
      For Each leadingTrivia In endOfFileToken.LeadingTrivia
        If leadingTrivia.IsKind(SyntaxKind.EndOfLineTrivia) Then Return False
      Next
      ' Checking if the last token is part of a LINQ query.
      Dim node = token.Parent
      Do
        If node.IsKind(SyntaxKind.QueryExpression) Then Return True
        If node Is statementNode Then Return False
        node = node.Parent
      Loop
    End Function
    ...
    Friend Shared Function IsCompleteSubmission(
                                                 tree As SyntaxTree
                                               ) As Boolean
      ...
      If options.Kind = SourceCodeKind.Regular Then Throw New ArgumentException(VBResources.SyntaxTreeIsNotASubmission)

      Dim languageVersion As LanguageVersion = options.LanguageVersion

      If Not tree.HasCompilationUnitRoot Then Return False
      ...
      For Each err In lastToken.GetDiagnostics()
        Select Case DirectCast(err.Code, ERRID)
               Case ERRID.ERR_UnterminatedStringLiteral
                    If Parser.CheckFeatureAvailability(languageVersion, Feature.MultilineStringLiterals) Then Return False
        End Select
      Next
