...
Namespace Microsoft.CodeAnalysis.VisualBasic.Symbols
  ...
  Friend Module SymbolExtensions
    ...
    ''' <summary> Does the compilation this symbol belongs to output to a winmdobj? </summary>
    <Extension>
    ...
    ''' <summary> String such as 'class', 'interface' etc that can be used in error messages. </summary>
    <Extension>
    ...
    ''' <summary> String "ReadOnly", "WriteOnly", or "" describing the kind of property. </summary>
    <Extension>
    Friend Function GetPropertyKindText(target As PropertySymbol) As String
      If target.IsWriteOnly Then Return SyntaxFacts.GetText(SyntaxKind.WriteOnlyKeyword)
      If target.IsReadOnly  Then Return SyntaxFacts.GetText(SyntaxKind.ReadOnlyKeyword)
      Return ""
    End Function
    ...
    Friend Function ToErrorMessageArgument(target As Symbol, Optional errorCode As ERRID = ERRID.ERR_None) As Object
      If target.Kind = SymbolKind.Namespace Then
         ...
         If ns.IsGlobalNamespace Then Return StringConstants.UnnamedNamespaceErrName
      End If
      If errorCode = ERRID.ERR_TypeConflict6 Then Return CustomSymbolDisplayFormatter.DefaultErrorFormat(target)
      Return target
    End Function

    ''' <summary> Checks if there is a name match with any type parameter. </summary>
    ''' <param name="this"/>
    ''' <param name="name"/>
    <Extension>
    Friend Function MatchesAnyName(this As ImmutableArray(Of TypeParameterSymbol), name As String) As Boolean
      For Each tp In this
        If IdentifierComparison.Comparer.Compare(name, tp.Name) = 0 Then Return True
      Next
      Return False
    End Function

    ''' <summary> Returns true if this symbols can overload another of the same kind. </summary>
    <Extension>
    Public Function IsOverloadable(symbol As Symbol) As Boolean
      Dim kind = symbol.Kind
      If kind = SymbolKind.Method    Then Return True
      If kind <> SymbolKind.Property Then Return False
    ...
    ''' <summary> Returns true if this property can overload another. </summary>
    <Extension()>
    Public Function IsOverloadable(propertySymbol As PropertySymbol) As Boolean
      Return Not propertySymbol.IsWithEvents
    End Function

    ''' <summary> Helper that tells if symbol has Overloads (hidebysig) on it </summary>
    <Extension>
    Friend Function IsOverloads(sym As Symbol) As Boolean
      ...
      Select Case sym.Kind
             Case SymbolKind.Method
                  ...
                  If method.IsOverloads Then Return True
             Case SymbolKind.Property
                  Dim prop = DirectCast(sym, PropertySymbol)
                  If prop.IsOverloads Then Return True
      End Select
      Return False
    End Function

    ''' <summary> Member that does not have Overloads, is considered Shadows (hidebyname) </summary>
    <Extension>
    ...
    ''' <summary> Is the symbol an instance member (i.e. access requires a receiver) </summary>
    <Extension>
    ...
    ''' <summary> Is this a member of a interface that requires implementation? </summary>
    <Extension>
      ...
      Select Case sym.Kind
             Case SymbolKind.Method, SymbolKind.Property, SymbolKind.Event
                  ...
                  Return sym.ContainingType.IsInterfaceType() AndAlso
                         Not sym.IsShared AndAlso sym.IsMustOverride
             Case Else
                  Return False
      End Select
    End Function
    ...
    ''' <summary> Return the overridden symbol for either a method or property. </summary>
    <Extension>
    Friend Function OverriddenMember(sym As Symbol) As Symbol
      Select Case sym.Kind
             Case SymbolKind.Method     : Return DirectCast(sym, MethodSymbol).OverriddenMethod
             Case SymbolKind.Property   : Return DirectCast(sym, PropertySymbol).OverriddenProperty
             Case SymbolKind.Event      : Return DirectCast(sym, EventSymbol).OverriddenEvent
             Case Else                  : Return Nothing
      End Select
    End Function

    ''' <summary> Return the arity of a member.  </summary>
    <Extension>
    Friend Function GetArity(symbol As Symbol) As Integer
      Select Case symbol.Kind
             Case SymbolKind.Method     : Return (DirectCast(symbol, MethodSymbol)).Arity
             Case SymbolKind.NamedType,
                  SymbolKind.ErrorType  : Return (DirectCast(symbol, NamedTypeSymbol)).Arity
             Case Else
                  Return 0
      End Select
    End Function
    ...
    Friend Function GetMeParameter(sym As Symbol) As ParameterSymbol
       Select Case sym.Kind
              Case SymbolKind.Method      : Return DirectCast(sym, MethodSymbol).MeParameter
              Case SymbolKind.Field       : Return DirectCast(sym, FieldSymbol).MeParameter
              Case SymbolKind.Property    : Return DirectCast(sym, PropertySymbol).MeParameter
              Case SymbolKind.Parameter   : Return Nothing
              Case Else                   : Throw ExceptionUtilities.UnexpectedValue(sym.Kind)
       End Select
    End Function
    ...
    ''' <summary> Is symbol a user-defined operator method. </summary>
    <Extension>
    ...
    ''' <summary> Does symbol or its containing type have Microsoft.CodeAnalysis.Embedded() attribute. </summary>
    <Extension>
    ...
    ''' <summary> Does symbol or its containing type have Microsoft.VisualBasic.Embedded() attribute. </summary>
    <Extension>
    ...
    ''' <summary> Gets the upper-level named type symbol, or returns Nothing if it does not exist. </summary>
    <Extension>
    Friend Function GetUpperLevelNamedTypeSymbol(symbol As Symbol) As NamedTypeSymbol
      ...
      If upperLevelType Is Nothing Then Return Nothing
    ...
    Friend Function GetDeclaringSyntaxNode(Of T As VisualBasicSyntaxNode)(this As Symbol) As T
      For Each node In this.DeclaringSyntaxReferences.Select(Function(d) d.GetSyntax())
        ...
        If node_T IsNot Nothing Then Return node_T
      Next
    ...
    Friend Function AsMember(Of T As Symbol)(origMember As T, type As NamedTypeSymbol) As T
      ...
      If type Is origMember.ContainingType Then Return origMember
    ...
    Friend Function EnsureVbSymbolOrNothing(Of TSource As ISymbol, TDestination As {Symbol, TSource})(symbol As TSource, paramName As String) As TDestination
      ...
      If vbSymbol Is Nothing AndAlso symbol IsNot Nothing Then Throw New ArgumentException(VBResources.NotAVbSymbol, paramName)
      Return vbSymbol
    End Function
