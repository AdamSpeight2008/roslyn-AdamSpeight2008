  ...
  Friend Class TypeUnification
    ...
    Public Shared Function CanUnify(containingGenericType As NamedTypeSymbol, t1 As TypeSymbol, t2 As TypeSymbol) As Boolean
      ...
      Dim result As Boolean = CanUnifyHelper(containingGenericType,
                              ...
                              substitution)
#If DEBUG Then
      Debug.Assert(Not result OrElse SubstituteAllTypeParameters(substitution, New TypeWithModifiers(t1)).IsSameType(SubstituteAllTypeParameters(substitution, New TypeWithModifiers(t2)), TypeCompareKind.IgnoreTupleNames))
#End If
      Return result
    End Function
...
#End If
    ...
    Private Shared Function CanUnifyHelper(containingGenericType As NamedTypeSymbol, t1 As TypeWithModifiers, t2 As TypeWithModifiers, ByRef substitution As TypeSubstitution) As Boolean
      If t1 = t2 Then Return True
      If t1.Type Is Nothing OrElse t2.Type Is Nothing Then Return False
      ...
      Select Case t1.Type.Kind
        Case SymbolKind.ArrayType
          If t2.Type.TypeKind <> t1.Type.TypeKind OrElse Not t1.CustomModifiers.SequenceEqual(t2.CustomModifiers) Then Return False
          ...
          If Not at1.HasSameShapeAs(at2) Then Return False
        ...
        Case SymbolKind.NamedType, SymbolKind.ErrorType
          ...
          If nt1.IsTupleType OrElse nt2.IsTupleType Then
            Return CanUnifyHelper(containingGenericType,
                   ...
                   substitution)
          End If
          If Not nt1.IsGenericType Then Return Not nt2.IsGenericType AndAlso nt1 = nt2
          If Not nt2.IsGenericType Then Return False
          Dim arity As Integer = nt1.Arity
          If nt2.Arity <> arity OrElse nt2.OriginalDefinition <> nt1.OriginalDefinition Then Return False
        ...
        Case SymbolKind.TypeParameter
          If t2.Type.SpecialType = SpecialType.System_Void Then Return False

          Dim tp1 = DirectCast(t1.Type, TypeParameterSymbol)
          If Contains(t2.Type, tp1) Then Return False
    ...
    Private Shared Function Contains(type As TypeSymbol, typeParam As TypeParameterSymbol) As Boolean
      Select Case type.Kind
        Case SymbolKind.ArrayType       : Return Contains((DirectCast(type, ArrayTypeSymbol)).ElementType, typeParam)
        Case SymbolKind.NamedType,
             SymbolKind.ErrorType       : Return NamedOrErrorType(type, typeParam)
        Case SymbolKind.TypeParameter   : Return type = typeParam
      End Select
      Return False
    End Function

        Private Shared Function NamedOrErrorType(type As TypeSymbol, typeParam As TypeParameterSymbol) As Boolean
