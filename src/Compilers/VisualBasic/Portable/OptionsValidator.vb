...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  ''' <summary> This class is used to validate the compiler options. </summary>
  Friend Module OptionsValidator

    ''' <summary> Parse the project level imports, adding errors to the errorBag as necessary. </summary>
    Friend Function ParseImports(
                                  importsClauses As IEnumerable(Of String),
      ...
      Dim parsedImportList As New Concurrent.ConcurrentBag(Of GlobalImport) 'New List(Of GlobalImport)
      Dim importList As SyntaxList(Of ImportsStatementSyntax) = tree.GetCompilationUnitRoot().Imports
      Parallel.For(0, importList.Count,
      Sub(i)
        ...
        If importsClauses.Count > 0 Then
           ...
           Dim errors = From diag In syntaxErrors Select import.MapDiagnostic(diag)
        ...
        End If
      End Sub)
      Return parsedImportList.OrderBy(Function(x) x.Name).ToArray()
    End Function
    ...
    ''' </summary>
    Friend Function IsValidNamespaceName(
                                          name As String
                                        ) As Boolean
    ...
    ''' </summary>
    Private Function IsValidRootNamespaceComponent(
                                                    name          As String,
                                                    start         As Integer,
                                                    [end]         As Integer,
                                                    allowEscaping As Boolean
                                                  ) As Boolean
      Debug.Assert(name IsNot Nothing)
