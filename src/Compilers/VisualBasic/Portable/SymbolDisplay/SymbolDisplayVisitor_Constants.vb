...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Partial Friend Class SymbolDisplayVisitor
    ...
    Protected Overrides Sub AddBitwiseOr()
      AddKeyword(SyntaxKind.OrKeyword, false, false)
    End Sub
    ...
    Protected Overrides Sub AddLiteralValue(
                                             type  As SpecialType,
                                             value As Object
                                           )
      ...
      Select Case type
              Case SpecialType.System_String    : SymbolDisplay.AddSymbolDisplayParts(builder, DirectCast(value, String))
              Case SpecialType.System_Char      : SymbolDisplay.AddSymbolDisplayParts(builder, DirectCast(value, Char))
              Case Else
    ...
    ''' <summary> Append a default argument (i.e. the default argument of an optional parameter). Assumed to be non-null. </summary>
    Private Sub AddConstantValue(
                                  type          As ITypeSymbol,
                                  constantValue As Object,
                         Optional preferNumericValueOrExpandedFlagsForEnum As Boolean = False
                                )
      If constantValue IsNot Nothing Then
        AddNonNullConstantValue(type, constantValue, preferNumericValueOrExpandedFlagsForEnum)
      Else
        AddKeyword(SyntaxKind.NothingKeyword, False, False)
      End If
    End Sub
