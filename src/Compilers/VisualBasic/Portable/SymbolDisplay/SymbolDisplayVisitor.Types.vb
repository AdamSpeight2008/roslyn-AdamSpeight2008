  ...
  Partial Friend Class SymbolDisplayVisitor
    ...
    Private Sub AddArrayRank(symbol As IArrayTypeSymbol)
      ...
      If symbol.Rank > 1 Then
         If insertStars Then AddPunctuation(SyntaxKind.AsteriskToken, False)
      Else
         ...
         If array IsNot Nothing AndAlso Not array.IsSZArray Then
            ...
            AddPunctuation(SyntaxKind.AsteriskToken, False)
         End If
      End If

      Dim i As Integer = 0
      While i < symbol.Rank - 1
        AddPunctuation(SyntaxKind.CommaToken, False)
        If insertStars Then AddPunctuation(SyntaxKind.AsteriskToken, False)
        i = i + 1
      End While
    ...
    Public Overrides Sub VisitDynamicType(symbol As IDynamicTypeSymbol)
      AddKeyword(SyntaxKind.ObjectKeyword, False, False)
    End Sub
    ...
    Public Overrides Sub VisitNamedType(symbol As INamedTypeSymbol)
      ...
      If format.MiscellaneousOptions.IncludesOption(SymbolDisplayMiscellaneousOptions.UseSpecialTypes) Then
        ...
        If Not format.MiscellaneousOptions.IncludesOption(SymbolDisplayMiscellaneousOptions.ExpandNullable) Then
           If ITypeSymbolHelpers.IsNullableType(symbol) AndAlso symbol IsNot symbol.OriginalDefinition Then
              ...
              AddPunctuation(SyntaxKind.QuestionToken, False)
              Return
           End If
        End If
      End If
      ...
      If CanShowDelegateSignature(symbol) Then
        If symbol.IsAnonymousType OrElse
          ...
          If invokeMethod.ReturnsVoid Then
            AddKeyword(SyntaxKind.SubKeyword, False, False)
          Else
            ...
            End If
            AddKeyword(SyntaxKind.FunctionKeyword, False, False)
          End If
          AddSpace()
        End If
      End If
      ...
      If ShouldVisitNamespace(containingSymbol) Then
        ...
        Dim emittedName As String = If( vbNamedTypeSymbol IsNot Nothing,If(vbNamedTypeSymbol.GetEmittedNamespaceName(), String.Empty),   String.Empty)
    ...
    Private Sub AddNameAndTypeArgumentsOrParameters(symbol As INamedTypeSymbol, noEscaping As Boolean)
      ...
      Else
         ...
         Else
            ...
            If missingCanonicalTypeSymbol IsNot Nothing Then symbolName = missingCanonicalTypeSymbol.FullTypeName
         End If
      End If

      If symbolName Is Nothing Then symbolName = symbol.Name
      ...
      Select Case symbol.TypeKind
             ...
             Case Else                  : Throw ExceptionUtilities.UnexpectedValue(symbol.TypeKind)
      End Select
      ...
      ElseIf symbol.Arity > 0 AndAlso format.GenericsOptions.IncludesOption(SymbolDisplayGenericsOptions.IncludeTypeParameters) AndAlso Not skipTypeArguments Then
         If isMissingMetadataType OrElse symbol.IsUnboundGenericType Then
            AddPunctuation(SyntaxKind.OpenParenToken, False)
            AddKeyword(SyntaxKind.OfKeyword, False, True)
            Dim i As Integer = 0
    ...
    Private Sub AddAnonymousTypeName(symbol As INamedTypeSymbol)
      Select Case symbol.TypeKind
             Case TypeKind.Class
                  ...
                  builder.Add(New SymbolDisplayPart(SymbolDisplayPartKind.ClassName, symbol, If(members.Length = 0, "<empty anonymous type>", $"<anonymous type: {members}>")))
    ...
    Private Sub AddTypeParameterVarianceIfRequired(symbol As ITypeParameterSymbol)
      If Not format.GenericsOptions.IncludesOption(SymbolDisplayGenericsOptions.IncludeVariance) Then Return
        Select Case symbol.Variance
               Case VarianceKind.In  : AddKeyword(SyntaxKind.InKeyword, False, True)
               Case VarianceKind.Out : AddKeyword(SyntaxKind.OutKeyword, False, True)
        End Select
    End Sub

    Private Sub AddTypeArguments(
      ...
      For i As Integer = 0 To typeArguments.Length - 1
        ...
        If typeArg.Kind = SymbolKind.TypeParameter Then
            ...
            If format.GenericsOptions.IncludesOption(SymbolDisplayGenericsOptions.IncludeTypeConstraints) Then AddTypeParameterConstraints(typeParam)
    ...
    Private Sub AddTypeParameterConstraints(typeParam As ITypeParameterSymbol)
      If Not isFirstSymbolVisited Then Return
      ...
      If constraintCount = 0 Then                Return
      AddKeyword(SyntaxKind.AsKeyword, True, True)

      If constraintCount > 1 Then AddPunctuation(SyntaxKind.OpenBraceToken, False)

      Dim needComma As Boolean = False
      If typeParam.HasReferenceTypeConstraint Then
        AddKeyword(SyntaxKind.ClassKeyword, False, False)
        needComma = True
      ElseIf typeParam.HasValueTypeConstraint Then
        AddKeyword(SyntaxKind.StructureKeyword, False, False)
        needComma = True
      End If
      ...
      If constraintCount > 1 Then AddPunctuation(SyntaxKind.CloseBraceToken, False)
