...
#Disable Warning CA1200 ' Avoid using cref tags with a prefix
    ''' <summary> Displays a symbol in the VisualBasic style. </summary>
...
#Enable Warning CA1200 ' Avoid using cref tags with a prefix
  ...
  Public Module SymbolDisplay
    ''' <summary> Displays a symbol in the Visual Basic style, based on a <see cref="SymbolDisplayFormat"/>. </summary>
    ...
    ''' <remarks> The return value is not expected to be syntactically valid Visual Basic. </remarks>
    ...
    ''' </remarks>
    Public Function ToMinimalDisplayString(
                                            symbol As ISymbol,
    ...
    ''' </remarks>
    Public Function ToDisplayParts(
                                    symbol As ISymbol,
                           Optional format As SymbolDisplayFormat = Nothing
                                  ) As ImmutableArray(Of SymbolDisplayPart)
    ...
    ''' Parts are not localized until they are converted to strings.
    ''' </remarks>
    Public Function ToMinimalDisplayParts(
                                           symbol As ISymbol,
                                           semanticModel As SemanticModel,
                                           position As Integer,
                                  Optional format As SymbolDisplayFormat = Nothing
    ...
    Private Function ToDisplayParts(
                                     symbol As ISymbol,
    ...
    Public Function FormatPrimitive(
                                     ...
                                     useHexadecimalNumbers As Boolean
                                   ) As String
      Dim options = ObjectDisplayOptions.None
      If quoteStrings Then options = options Or ObjectDisplayOptions.UseQuotes Or ObjectDisplayOptions.EscapeNonPrintableCharacters
    ...
    Friend Sub AddSymbolDisplayParts(
      ...
      Next
      If lastKind >= 0 Then parts.Add(New SymbolDisplayPart(DirectCast(lastKind, SymbolDisplayPartKind), Nothing, sb.ToString()))
      pooledBuilder.Free()
    End Sub
