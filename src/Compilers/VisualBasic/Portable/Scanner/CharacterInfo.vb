...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Partial Public Class SyntaxFacts
    ...
    Friend Const FULLWIDTH_UNDERSCORE           = ChrW(s_fullwidth + AscW("_"c))  REM ＇
    ...
    Friend Shared Function IsSingleQuote(c As Char) As Boolean
        ...
        Return c = "'"c OrElse (c >= LEFT_SINGLE_QUOTATION_MARK AndAlso c.IsAnyOf(FULLWIDTH_APOSTROPHE,
    ...
    Friend Shared Function IsDoubleQuote(c As Char) As Boolean
        ...
        ' // IME editors paste them in. This isn't really technically correct
        ' // because we ignore the left-ness or right-ness, but see VS 170991
        Return c = """"c OrElse (c >= LEFT_DOUBLE_QUOTATION_MARK AndAlso c.IsAnyOf(FULLWIDTH_QUOTATION_MARK, LEFT_DOUBLE_QUOTATION_MARK, RIGHT_DOUBLE_QUOTATION_MARK))
    End Function
    ...
    Public Shared Function IsUnderscore(c As Char) As Boolean
        ...
        Return c.IsAnyOf("_"c, FULLWIDTH_UNDERSCORE)
    End Function
    ...
    Friend Shared Function IntegralLiteralCharacterValue( Digit As Char ) As Byte
        ...
        ElseIf Digit.InRange("A"c,"F"c) Then
            Return CByte(u + (10 - AscW("A"c)))
        Else
            Debug.Assert(Digit.InRange("a"c,"f"c), "Surprising digit.")
            Return CByte(u + (10 - AscW("a"c)))
        End If
    End Function
    
    Friend Shared Function BeginsBaseLiteral(c As Char) As Boolean
        Return c.IsAnyOf("H"c, "O"c, "B"c, "h"c, "o"c, "b"c) OrElse
    ...
    Public Shared Function IsIdentifierPartCharacter(c As Char) As Boolean
      Return If( c < ChrW(128), IsNarrowIdentifierCharacter(Convert.ToUInt16(c)),IsWideIdentifierCharacter(c))
    End Function
    ...
    Friend Shared Function IsBinaryDigit(c As Char) As Boolean
        Return c.InRange("0"c,"1"c) OrElse c.InRange(FULLWIDTH_DIGIT_ZERO, FULLWIDTH_DIGIT_ONE)
    End Function
    
    Friend Shared Function IsOctalDigit(c As Char) As Boolean
        Return c.InRange("0"c,"7"c) OrElse c.InRange(FULLWIDTH_DIGIT_ZERO, FULLWIDTH_DIGIT_SEVEN)
    End Function
    
    Friend Shared Function IsDecimalDigit(c As Char) As Boolean
        Return c.InRange("0"c,"9"c) OrElse c.InRange(FULLWIDTH_DIGIT_ZERO,FULLWIDTH_DIGIT_NINE)
    End Function
    
    Friend Shared Function IsHexDigit(c As Char) As Boolean
        Return IsDecimalDigit(c) OrElse
               c.InRange("a"c,"f"c) OrElse
               c.InRange("A"c,"F"c) OrElse
               c.InRange(FULLWIDTH_LATIN_SMALL_LETTER_A,FULLWIDTH_LATIN_SMALL_LETTER_F) OrElse
               c.InRange(FULLWIDTH_LATIN_CAPITAL_LETTER_A, FULLWIDTH_LATIN_CAPITAL_LETTER_F)
    End Function
    ...
    Friend Shared Function MatchOneOrAnotherOrFullwidth(ch As Char, one As Char, another As Char) As Boolean
      ...
      Return MatchOneOrAnother(ch, one, another)
    End Function
    ...
    Friend Shared Function IsPropCombining(CharacterProperties As UnicodeCategory) As Boolean
      Return CharacterProperties >= UnicodeCategory.NonSpacingMark AndAlso CharacterProperties <= UnicodeCategory.EnclosingMark
    End Function
