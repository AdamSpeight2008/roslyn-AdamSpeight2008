...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Public Class VisualBasicCommandLineParser
    ...
    ''' <summary> Gets the standard Visual Basic source file extension </summary>
    ...
    ''' <summary> Gets the standard Visual Basic script file extension. </summary>
    ...
    Public Shadows Function Parse(
                                   args As IEnumerable(Of String),
                                   baseDirectory As String,
                                   sdkDirectory As String,
                          Optional additionalReferenceDirectories As String = Nothing
                                 ) As VisualBasicCommandLineArguments

      Const GenerateFileNameForDocComment As String = "USE-OUTPUT-NAME"

      Dim _vb_ As (RuntimePath As String, embedRuntime As Boolean, IncludeRuntimeRef As Boolean) = (Nothing, True, False)      
     
      'Dim vbRuntimePath             = Nothing
      'Dim includeVbRuntimeReference = True
      'Dim embedVbCoreRuntime        = False
      ...
      Dim hasSourceFiles                As Boolean = False
      Dim embedAllSourceFiles           As Boolean = False
      ...
      Dim checksumAlgorithm             As SourceHashAlgorithm  = SourceHashAlgorithm.Sha1
      ...
      Dim keyFileSearchPaths As New List(Of String)()
      ...
      For Each arg In flattenedArgs
        ...
        Select Case name
               ...
               Case "optionstrict"
                    ...
                    Continue For
               Case "optionstrict+"
                       SwitchNeedsBoolean(value, diagnostics, optionStrict, OptionStrict.On, "optionStrict")
              ...
              Case "optionstrict-"
                    SwitchNeedsBoolean(value, diagnostics, optionStrict, OptionStrict.Off, "optionStrict")
              ...
              Case "optionexplicit", "optionexplicit+"
                    SwitchNeedsBoolean(value, diagnostics, optionExplicit, True, "optionexplicit")
                  'If value IsNot Nothing Then
              ...
              Case "optioninfer", "optioninfer+"
                   SwitchNeedsBoolean(value, diagnostics, optionInfer, True, "optioninfer")
                    'If value IsNot Nothing Then
               ...
               Case "checksumalgorithm"
                ...
                Case "removeintchecks", "removeintchecks+"
                     SwitchNeedsBoolean(value, diagnostics, checkOverflow, False, "removeintchecks")
                ...
                Case "removeintchecks-"
                     SwitchNeedsBoolean(value, diagnostics, checkOverflow, True, "removeintchecks")
                ...
                Case "preferreduilang"
                     ...
                     Else
                       ...
                       End Try
                       If preferredUILang Is Nothing Then AddDiagnostic(diagnostics, ERRID.WRN_BadUILang, value)
                     End if
                     Continue For
...
#End If
        ...
        If IsScriptCommandLineParser Then
           Select Case name
                  Case "i", "i+"
                       SwitchNeedsBoolean(value, diagnostics, interactiveMode, True, "i")
                       Continue For
                  Case "i-"
                       SwitchNeedsBoolean(value, diagnostics, interactiveMode, False, "i")
                       Continue For
                  Case "loadpath",
                       "loadpaths"
                       If String.IsNullOrEmpty(value) Then
        ...
        Else
           Select Case name
                  ...
                  Case "refonly", "refonly+"
                       SwitchNeedsBoolean(value, diagnostics, refOnly, true, "refonly")
                       Continue For
                  ...
                  Case "doc"
                       ...
                       Continue For
                  Case "doc+"
                       If value IsNot Nothing Then AddDiagnostic(diagnostics, ERRID.ERR_SwitchNeedsBool, "doc")
                  ...
                  Case "instrument"
                       ...
                       Else
                         For Each instrumentationKind As InstrumentationKind In ParseInstrumentationKinds(value, diagnostics)
                           If Not instrumentationKinds.Contains(instrumentationKind) Then instrumentationKinds.Add(instrumentationKind)
                         Next
                       End If
                       Continue For
                  Case "recurse"
                       ...
                       Else
                          ...
                          If sourceFiles.Count > before Then hasSourceFiles = True
                       End If
                       Continue For
                  Case "addmodule"
                       If String.IsNullOrEmpty(value) Then
                       ...
                       Else
                          ...
                          metadataReferences.AddRange( ParseSeparatedPaths(value).Select( Function(path) New CommandLineReference(path, New MetadataReferenceProperties(MetadataImageKind.Module))))
                       End If
                       Continue For
                  Case "l", "link"
                  ...
                  Case "res", "resource"
                       ...
                       If embeddedResource IsNot Nothing Then managedResources.Add(embeddedResource)
                       Continue For
                  Case "linkres", "linkresource"
                       Dim linkedResource = ParseResourceDescription(name, value, baseDirectory, diagnostics, embedded:=False)
                       If linkedResource IsNot Nothing Then managedResources.Add(linkedResource)
                       Continue For
                  Case "sourcelink"
                       value = RemoveQuotesAndSlashes(value)
                       If String.IsNullOrEmpty(value) Then
                  ...
                  Case "debug"
                       ...
                       If value IsNot Nothing Then
                          Select Case value.ToLower()
                                 Case "full",
                                      "pdbonly"     : debugInformationFormat = If(PathUtilities.IsUnixLikePlatform, DebugInformationFormat.PortablePdb, DebugInformationFormat.Pdb)
                                 Case "portable"    : debugInformationFormat = DebugInformationFormat.PortablePdb
                                 Case "embedded"    : debugInformationFormat = DebugInformationFormat.Embedded
                                 Case Else          : AddDiagnostic(diagnostics, ERRID.ERR_InvalidSwitchValue, "debug", value)
                          End Select
                       End If
                       emitPdb = True
                       Continue For
                  Case "debug+"
                       SwitchNeedsBoolean(value, diagnostics, emitPdb, True, "debug")
                       Continue For
                  Case "debug-"
                        SwitchNeedsBoolean(value, diagnostics, emitPdb, false, "debug")

                  Case "optimize", "optimize+"
                       SwitchNeedsBoolean(value, diagnostics, optimize, True, "optimize")
                       Continue For
                  Case "optimize-"
                       SwitchNeedsBoolean(value, diagnostics, optimize, False, "optimize")
                       Continue For


                  Case "deterministic", "deterministic+"
                       SwitchNeedsBoolean(value, diagnostics, deterministic, True, name)
                       Continue For

                  Case "deterministic-"
                       SwitchNeedsBoolean(value, diagnostics, deterministic, false, name)
                       Continue For
          #Region "ConcurrentBuild"
                  Case "parallel+", "p+"
                       SwitchNeedsBoolean(value, diagnostics, concurrentBuild, true, name.Substring(0, name.Length - 1))
                       Continue For
                  Case "parallel-", "p-"
                       SwitchNeedsBoolean(value, diagnostics, concurrentBuild, false, name.Substring(0, name.Length - 1))
                       Continue For
                  Case "parallel", "p"
                       SwitchNeedsBoolean(value, diagnostics, concurrentBuild, true, name)
                       Continue For
          #End Region
                 Case "warnaserror", "warnaserror+"
                       If value Is Nothing Then
                          ...
                          For Each pair In specificDiagnosticOptionsFromRuleSet
                            If pair.Value = ReportDiagnostic.Warn Then specificDiagnosticOptionsFromGeneralArguments.Add(pair.Key, ReportDiagnostic.Error)
                          Next
                       Else
                  ...
                  Case "warnaserror-"
                       If value Is Nothing Then
                          If generalDiagnosticOption <> ReportDiagnostic.Suppress Then generalDiagnosticOption = ReportDiagnostic.Default
                          specificDiagnosticOptionsFromGeneralArguments.Clear()
                       Else
                  ...
                  Case "nowarn"
                       If value Is Nothing Then
                          ...
                          specificDiagnosticOptionsFromGeneralArguments.Clear()
                          For Each pair In specificDiagnosticOptionsFromRuleSet
                             If pair.Value <> ReportDiagnostic.Error Then specificDiagnosticOptionsFromGeneralArguments.Add(pair.Key, ReportDiagnostic.Suppress)
                          Next
                          Continue For
                       End If
                  ...
                  Case "langversion"
                       ...
                       Else
                          If Not TryParse(value, languageVersion) Then AddDiagnostic(diagnostics, ERRID.ERR_InvalidSwitchValue, "langversion", value)
                       End If
                       Continue For


                  Case "delaysign", "delaysign+"
                       SwitchNeedsBoolean(value, diagnostics, delaySignSetting, true, "delaysign")
                       Continue For
                  Case "delaysign-"
                       SwitchNeedsBoolean(value, diagnostics, delaySignSetting, false, "delaysign")
                       Continue For

                  Case "publicsign", "publicsign+"
                       SwitchNeedsBoolean(value, diagnostics, publicsign, true, "publicsign")
                       Continue For

                  Case "publicsign-"
                       SwitchNeedsBoolean(value, diagnostics, publicsign, false, "publicsign")
                       Continue For
                  ...
                  Case "keyfile"
                       ...
                       ArgumentRequired(String.IsNullOrEmpty(value), diagnostics, keyFileSetting, value,"keyfile", ":<file>")
                  ...
                  #Region "utf8Output"
                  Case "utf8output"
                       SwitchNeedsBoolean(value, diagnostics, utf8output, True, "utf8output")
                       Continue For
                  Case "utf8output+"
                       SwitchNeedsBoolean(value, diagnostics, utf8output, True, "utf8output+")
                       Continue For
                  Case "utf8output-"
                       SwitchNeedsBoolean(value, diagnostics, utf8output, False, "utf8output-")
                       Continue For
                  #End Region
                  ...
                  Case "m", "main"
                       ...
                       value = RemoveQuotesAndSlashes(value)
                       ArgumentRequired(String.IsNullOrEmpty(value), diagnostics, mainTypeName, value,name, ":<class>")
                  ...
                  Case "touchedfiles"
                       Dim unquoted = RemoveQuotesAndSlashes(value)
                       ArgumentRequired(String.IsNullOrEmpty(unquoted), diagnostics, touchedFilesPath, unquoted, name, ":<touchedfiles>")
                       Continue For
                  ...
                  Case "pathmap"
                       ' "/pathmap:K1=V1,K2=V2..."
                       Dim unquoted = RemoveQuotesAndSlashes(value)
                       If unquoted = Nothing Then Exit Select
                  ...
                  Case "vbruntime"
                       If value Is Nothing Then
                          _vb_ = (RuntimePath:= Nothing, embedRuntime:= False, IncludeRuntimeRef:= True)
                       Else
                          ' NOTE: that Dev11 does not report errors on empty or invalid file specified
                          _vb_ = (RuntimePath:= RemoveQuotesAndSlashes(value), embedRuntime:= False, IncludeRuntimeRef:= True)
                       End If
                       Continue For
                  ...
                  Case "vbruntime-"
                       If value IsNot Nothing Then Exit Select
                        _vb_ = (RuntimePath:=Nothing, embedRuntime:= False, IncludeRuntimeRef:= False)
                       Continue For

                  Case "vbruntime*"
                       If value IsNot Nothing Then Exit Select
                       _vb_ = (RuntimePath:= Nothing, embedRuntime:= True, IncludeRuntimeRef:= False)
                       Continue For

                  Case "platform"
                       value = RemoveQuotesAndSlashes(value)
                       ArgumentRequired(value IsNot Nothing, diagnostics, platform, ParsePlatform(name, value, diagnostics) , "platform", ":<string>")
                       'If value IsNot Nothing Then
      ...
      specificDiagnosticOptions.FillFrom(specificDiagnosticOptionsFromGeneralArguments).
                                FillFrom(specificDiagnosticOptionsFromSpecificArguments).
                                FillFrom(specificDiagnosticOptionsFromNoWarnArguments)


      If refOnly AndAlso outputRefFileName IsNot Nothing Then AddDiagnostic(diagnostics, ERRID.ERR_NoRefOutWhenRefOnly)

      If outputKind = OutputKind.NetModule AndAlso (refOnly OrElse outputRefFileName IsNot Nothing) Then AddDiagnostic(diagnostics, ERRID.ERR_NoNetModuleOutputWhenRefOutOrRefOnly)
      ...
      ' Prepare SDK PATH
      If sdkDirectory IsNot Nothing AndAlso sdkPaths.Count = 0 Then sdkPaths.Add(sdkDirectory)
      ...
      If Not noStdLib Then
         ...
         Else
            metadataReferences.Add( New CommandLineReference(systemDllPath, New MetadataReferenceProperties(MetadataImageKind.Assembly)))
         End If
      ...
      If _vb_.IncludeRuntimeRef Then
         If _vb_.RuntimePath Is Nothing Then
            ...
            Else
               metadataReferences.Add( New CommandLineReference(msVbDllPath, New MetadataReferenceProperties(MetadataImageKind.Assembly)))
            End If
         Else
            metadataReferences.Add(New CommandLineReference(_vb_.RuntimePath, New MetadataReferenceProperties(MetadataImageKind.Assembly)))
         End If
      End If

          ' add additional reference paths if specified
      If Not String.IsNullOrWhiteSpace(additionalReferenceDirectories) Then libPaths.AddRange(ParseSeparatedPaths(additionalReferenceDirectories))
      ' Build search path
      ...
      If publicSign AndAlso Not String.IsNullOrWhiteSpace(keyFileSetting) Then keyFileSetting = ParseGenericPathToFile(keyFileSetting, diagnostics, baseDirectory)
      ValidateWin32Settings(noWin32Manifest, win32ResourceFile, win32IconFile, win32ManifestFile, outputKind, diagnostics)

      If sourceLink IsNot Nothing And Not emitPdb Then AddDiagnostic(diagnostics, ERRID.ERR_SourceLinkRequiresPdb)
      If embedAllSourceFiles Then embeddedFiles.AddRange(sourceFiles)
      If embeddedFiles.Count > 0 And Not emitPdb Then AddDiagnostic(diagnostics, ERRID.ERR_CannotEmbedWithoutPdb)
      ...
      If Not IsScriptCommandLineParser  AndAlso Not hasSourceFiles AndAlso Not managedResources.IsEmpty() AndAlso
         outputFileName = Nothing AndAlso Not flattenedArgs.IsEmpty() Then
      ...
      Dim options = New VisualBasicCompilationOptions(
            ...
            embedVbCoreRuntime:= _vb_.embedRuntime,
    ...
    Private Sub ArgumentRequired(Of T)(
                                        condition As Boolean,
                                        diagnostics As List(Of Diagnostic),
                                  byref _option_    As T,
                                        value       As T,
                                        text        As string, args As String)
      If condition Then
         AddDiagnostic(diagnostics, ERRID.ERR_ArgumentRequired, text, args)
      Else
         _option_ = value
      End If
    End Sub

    Private Function LoadCoreLibraryReference(
                                               sdkPaths      As List(Of String),
                                               baseDirectory As String
                                             ) As CommandLineReference?
      ...
      If systemRuntimePath IsNot Nothing Then
         If msCorLibPath Is Nothing Then Return New CommandLineReference(systemRuntimePath, New MetadataReferenceProperties(MetadataImageKind.Assembly))
         ...
         Try
           Using metadata = AssemblyMetadata.CreateFromFile(systemRuntimePath)
             ...
             If metadata.GetModules()(0).Module.IsLinkedModule AndAlso  metadata.GetAssembly().AssemblyReferences.Length = 0 Then
    ...
    Private Shared Function FindFileInSdkPath(
                                               sdkPaths      As List(Of String),
                                               fileName      As String,
                                               baseDirectory As String
                                             ) As String
      For Each path In sdkPaths
        ...
        If absolutePath Is Nothing Then Continue For
    ...
    Private Shared Function GetWin32Setting(
                                             arg         As String,
                                             value       As String,
                                             diagnostics As List(Of Diagnostic)
                                           ) As String
      ...
      Else
         ...
         If Not String.IsNullOrWhiteSpace(noQuotes) Then Return noQuotes
         AddDiagnostic(diagnostics, ERRID.ERR_ArgumentRequired, arg, ":<file>")
      End If
      Return Nothing
    End Function

    Private Shared Function BuildSearchPaths(
                                              baseDirectory As String,
                                              sdkPaths      As List(Of String),
                                              responsePaths As List(Of String),
                                              libPaths      As List(Of String)
    ...
    Private Shared Sub AddNormalizedPaths(
    ...
    Private Shared Sub ValidateWin32Settings(
                                              noWin32Manifest As Boolean,
                                              win32ResSetting As String,
                                              win32IconSetting As String,
                                              win32ManifestSetting As String,
                                              outputKind As OutputKind,
                                              diagnostics As List(Of Diagnostic)
                                            )

        If noWin32Manifest AndAlso (win32ManifestSetting IsNot Nothing) Then AddDiagnostic(diagnostics, ERRID.ERR_ConflictingManifestSwitches)
        If win32ResSetting IsNot Nothing Then
            If win32IconSetting     IsNot Nothing Then AddDiagnostic(diagnostics, ERRID.ERR_IconFileAndWin32ResFile)
            If win32ManifestSetting IsNot Nothing Then AddDiagnostic(diagnostics, ERRID.ERR_CantHaveWin32ResAndManifest)
        End If
        If win32ManifestSetting IsNot Nothing AndAlso outputKind.IsNetModule() Then AddDiagnostic(diagnostics, ERRID.WRN_IgnoreModuleManifest)
    End Sub

    Private Shared Function ParseTarget(
      ...
      Select Case If(value, "").ToLowerInvariant()
             ...
             Case "module"           : Return OutputKind.NetModule
             Case "appcontainerexe"  : Return OutputKind.WindowsRuntimeApplication
             Case "winmdobj"         : Return OutputKind.WindowsRuntimeMetadata
             Case ""
    ...
    Friend Shared Function ParseAssemblyReferences(name As String, value As String, diagnostics As IList(Of Diagnostic), embedInteropTypes As Boolean) As IEnumerable(Of CommandLineReference)
      If Not String.IsNullOrEmpty(value) Then Return ParseSeparatedPaths(value). Select(Function(path) New CommandLineReference(path, New MetadataReferenceProperties(MetadataImageKind.Assembly, embedInteropTypes:=embedInteropTypes)))
    ...
    Private Shared Function ParseAnalyzers(
      ...
      If Not String.IsNullOrEmpty(value) Then Return ParseSeparatedPaths(value).Select(Function(path) New CommandLineAnalyzerReference(path))
      ' TODO: localize <file_list>?
      AddDiagnostic(diagnostics, ERRID.ERR_ArgumentRequired, name, ":<file_list>")
    ...
    Friend Overloads Shared Function ParseResourceDescription(
                                                               name As String,
    ...
    Private Shared Sub ParseGlobalImports(
                                           value            As String,
                                           globalImports    As List(Of GlobalImport),
                                           errors           As List(Of Diagnostic)
                                                    )
    ...
    Private Shared Function PublicSymbolsToInternalDefines(
                                                            symbols           As IEnumerable(Of KeyValuePair(Of String, Object)),
                                                            diagnosticBuilder As ArrayBuilder(Of Diagnostic)
                                                          ) As ImmutableDictionary(Of String, InternalSyntax.CConst)
      ...
      If symbols IsNot Nothing Then
         For Each symbol In symbols
           ...
           If constant Is Nothing Then diagnosticBuilder.Add(Diagnostic.Create(VisualBasic.MessageProvider.Instance, ERRID.ERR_InvalidPreprocessorConstantType, symbol.Key, symbol.Value.GetType()))
           ...
           Do
             ...
             If tokens.Current.Position > 0 AndAlso Not IsSeparatorOrEndOfFile(tokens.Current) Then
                ...
                While Not IsSeparatorOrEndOfFile(tokens.Current)
                  ...
                  tokens.MoveNext()
                End While
                diagnosticBuilder.Add( New DiagnosticWithInfo(
             ...
             While tokens.Current.Kind.IsAny(SyntaxKind.CommaToken, SyntaxKind.ColonToken)
             ...
             If tokens.Current.Kind = SyntaxKind.EqualsToken Then
                ...
                Dim value As InternalSyntax.CConst = InternalSyntax.ExpressionEvaluator.EvaluateExpression( DirectCast(expression.Green, InternalSyntax.ExpressionSyntax), defines)
             ...
             ElseIf tokens.Current.Kind.IsAny(SyntaxKind.CommaToken, SyntaxKind.ColonToken, SyntaxKind.EndOfFileToken) Then
    ...
    Private Shared Iterator Function ParseInstrumentationKinds(value As String, diagnostics As IList(Of Diagnostic)) As IEnumerable(Of InstrumentationKind)
      ...
      For Each instrumentationKindStr In instrumentationKindStrs
        Select Case instrumentationKindStr.ToLower()
               Case "testcoverage"  : Yield InstrumentationKind.TestCoverage
               Case Else            : AddDiagnostic(diagnostics, ERRID.ERR_InvalidInstrumentationKind, instrumentationKindStr)
        End Select
      Next
    End Function

    Private Shared Function IsSeparatorOrEndOfFile(token As SyntaxToken) As Boolean
      Return token.Kind.IsAny(SyntaxKind.EndOfFileToken, SyntaxKind.ColonToken, SyntaxKind.CommaToken)
    End Function
    ...
    Private Shared Function ParsePlatform(name As String, value As String, errors As List(Of Diagnostic)) As Platform
      ...
      Else
         Select Case value.ToLowerInvariant()
                ...
                Case "itanium"              : Return Platform.Itanium
                Case "anycpu"               : Return Platform.AnyCpu
                Case "anycpu32bitpreferred" : Return Platform.AnyCpu32BitPreferred
                Case "arm"                  : Return Platform.Arm
                Case "arm64"                : Return Platform.Arm64
         End Select
    ...
    Private Shared Function ParseBaseAddress(name As String, value As String, errors As List(Of Diagnostic)) As ULong
      ...
      Else
         ...
         If ULong.TryParse(parseValue, NumberStyles.HexNumber, CultureInfo.InvariantCulture, baseAddress) Then  Return baseAddress
    ...
    ''' <summary> Parses the warning option. </summary>
    ''' <param name="value">The value for the option.</param>
    Private Shared Iterator Function ParseWarnings(value As String) As IEnumerable(Of String)
      ...
      For Each id In values
         ...
         If UShort.TryParse(id, NumberStyles.Integer, CultureInfo.InvariantCulture, number) AndAlso
            ...
            Yield VisualBasic.MessageProvider.Instance.GetIdForErrorCode(CInt(number))
         Else
    ...
    Private Shared Sub AddWarnings(d As IDictionary(Of String, ReportDiagnostic), kind As ReportDiagnostic, items As IEnumerable(Of String))
      For Each id In items
        ...
        If d.TryGetValue(id, existing) Then
           ...
           If existing <> ReportDiagnostic.Suppress Then d(id) = kind
        Else
           d.Add(id, kind)
        End If
      Next
    ...
    ''' ]]>
    ''' </remarks>
    Private Sub GetCompilationAndModuleNames(
                                              diagnostics           As List(Of Diagnostic),
                                              kind                  As OutputKind,
      ...
      Else
         ...
         If kind.IsNetModule() Then
            If ext.Length = 0 Then outputFileName = outputFileName & ".netmodule"
         Else
      ...
      Else
         If moduleAssemblyName IsNot Nothing Then AddDiagnostic(diagnostics, ERRID.ERR_NeedModule)
         compilationName = simpleName
      End If

      If moduleName Is Nothing Then moduleName = outputFileName
