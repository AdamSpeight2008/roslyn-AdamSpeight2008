  ...
  Friend NotInheritable Class CommandLineDiagnosticFormatter
    ...
    Public Overrides Function Format(diagnostic As Diagnostic, Optional formatter As IFormatProvider = Nothing) As String
      ...
      If Not diagnosticSpanOpt.HasValue OrElse
         ...
         If diagnostic.Location <> Location.None Then diagnostic = diagnostic.WithLocation(Location.None)
         ...
         Return VisualBasicCompiler.VbcCommandLinePrefix & MyBase.Format(diagnostic, formatter)
      End If
      ...
      While (line.Start < sourceSpanEnd)
        ...
        For position = Math.Min(sourceSpanStart, line.Start) To Math.Min(line.End, sourceSpanStart) - 1
                              AppendOneOrFour(sb, " "c, text(position)= vbTab)
    ...
    Private Function AppendOneOrFour(sb As StringBuilder, ch As Char, cond As Boolean) As StringBuilder
      Return sb.Append(ch,If(cond,4,1))
    End Function
