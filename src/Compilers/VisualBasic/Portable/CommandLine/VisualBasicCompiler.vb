...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Friend MustInherit Class VisualBasicCompiler
    ...
    Protected Sub New(
                       parser           As VisualBasicCommandLineParser,
                       responseFile     As String,
                       args             As String(),
                       buildPaths       As BuildPaths,
                       additionalReferenceDirectories As String,
                       analyzerLoader   As IAnalyzerAssemblyLoader
                     )
    ...
    Public Overrides Function CreateCompilation(
                                                 consoleOutput      As TextWriter,
                                                 touchedFilesLogger As TouchedFileLogger,
                                                 errorLogger        As ErrorLogger
                                               ) As Compilation
      ...
      If Arguments.CompilationOptions.ConcurrentBuild Then
         Parallel.For(0, sourceFiles.Length,UICultureUtilities.WithCurrentUICulture(Of Integer)(
                      Sub(i As Integer)
      ...
      If hadErrors Then Return Nothing
      ...
      If ReportErrors(diagnostics, consoleOutput, errorLogger) Then Return Nothing
      If Arguments.OutputLevel = OutputLevel.Verbose Then PrintReferences(resolvedReferences, consoleOutput)
      Dim xmlFileResolver As New LoggingXmlFileResolver(Arguments.BaseDirectory, touchedFilesLogger)

      ' TODO: support for #load search paths
      Dim sourceFileResolver As New LoggingSourceFileResolver(ImmutableArray(Of String).Empty, Arguments.BaseDirectory, Arguments.PathMap, touchedFilesLogger)

      Dim loggingFileSystem As New LoggingStrongNameFileSystem(touchedFilesLogger)
    ...
    Private Sub PrintReferences(
                                 resolvedReferences As List(Of MetadataReference),
                                 consoleOutput      As TextWriter
                               )
    ...
    Friend Overrides Function SuppressDefaultResponseFile(
    ...
    ''' <summary> Print compiler logo </summary>
    ...
    ''' <summary> Print Commandline help message (up to 80 English characters per line) </summary>
    ...
    Public Overrides Sub PrintLangVersions(
                                            consoleOutput As TextWriter
                                          )
    ...
    Protected Overrides Function TryGetCompilerDiagnosticCode(
                                                               diagnosticId As String,
                                                         ByRef code         As UInteger
                                                             ) As Boolean
    ...
    Protected Overrides Function ResolveAnalyzersFromArguments(
                                                                diagnostics     As List(Of DiagnosticInfo),
                                                                messageProvider As CommonMessageProvider
                                                              ) As ImmutableArray(Of DiagnosticAnalyzer)
    ...
    Protected Overrides Sub ResolveEmbeddedFilesFromExternalSourceDirectives(
      ...
      For Each directive As ExternalSourceDirectiveTriviaSyntax In tree.GetRoot().GetDirectives(Function(d) d.Kind() = SyntaxKind.ExternalSourceDirectiveTrivia)

        If directive.ExternalSource.IsMissing Then Continue For
