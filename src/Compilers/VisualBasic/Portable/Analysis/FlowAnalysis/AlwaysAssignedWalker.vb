...
Namespace Microsoft.CodeAnalysis.VisualBasic
  ...
  Friend Class AlwaysAssignedWalker
      ...
      Get
        ...
        If (_endOfRegionState.Reachable) Then
           For Each i In _endOfRegionState.Assigned.TrueBits
             If (i >= variableBySlot.Length) Then Continue For

             Dim v = variableBySlot(i)
             If v.Exists AndAlso v.Symbol.Kind <> SymbolKind.Field Then result.Add(v.Symbol)
           Next
        End If
    ...
    Protected Overrides Sub ResolveBranch _
            (
              pending           As AbstractFlowPass(Of DataFlowPass.LocalState).PendingBranch,
              label             As LabelSymbol,
              target            As BoundLabelStatement,
        ByRef labelStateChanged As Boolean
            )
    ...
    Protected Overrides Sub WriteArgument(arg As BoundExpression, isOut As Boolean)
      ...
      If isOut Then MyBase.WriteArgument(arg, isOut)
    End Sub
